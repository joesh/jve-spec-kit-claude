...
### Implied Gap Clamp Attribution (2025-12-11)
- Rule 8.5 now drives clamp reporting: whenever a ripple drags items on tracks with no explicit selections, the constraint solver records the implied bracket (`gap_before`/`gap_after` on the shifted clip itself) instead of the stationary neighbor. These keys are stored in `global_min_edge_keys`/`global_max_edge_keys`, promoted to `forced_clamped_edges`, and surfaced in the dry-run payload so the renderer can highlight the actual blocking handle.
- `tests/test_batch_ripple_implied_gap_clamp.lua` covers the BatchRippleEdit payload by synthesizing two clips on V2 with a tiny gap and dragging a V1 gap edge far enough left that the V2 gap collapses. The dry run must now report `clip_v2_shift:gap_before` as the limiter while leaving the dragged bracket untouched.
- `tests/test_timeline_implied_gap_clamp_color.lua` verifies the UI path end-to-end: the timeline renderer requests the same dry run, draws the implied handle for the V2 track, and confirms only that handle switches to the limit color while the dragged edge on V1 stays green. This prevents the prior regression where every selected bracket turned red whenever an implied clamp fired.
