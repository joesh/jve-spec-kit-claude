...
| **R012** | Lua | `db:prepare(...)` followed by `stmt:next()` without intervening `stmt:exec()` | lsqlite3 needs `exec()` between bind and the first `next()`; without it `next()` is false from the start and the iterator silently returns zero rows. 2026-06-03 found this in two spec-023 files (`connect_to_resolve_project.lua::load_clips_on_track`, `payload_builder.lua::load_clips_for_track`), producing "0 JVE clip(s)" on populated sequences. Safe alternative: `database.select_rows(conn, sql, params, row_mapper)` encapsulates the full prepare→bind→exec→iter→finalize cycle. | 2026-06-03 |
