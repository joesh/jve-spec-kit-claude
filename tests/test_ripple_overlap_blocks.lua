...
local function seed_db(db_path)
    ...
    local seed = string.format([[
        ...
        VALUES ('media1', 'default_project', 'Media', 5000, 30.0, 1920, 1080);
...
local function fetch_start(db_conn, id)
    local stmt = db_conn:prepare("SELECT start_time FROM clips WHERE id = ?")
...
local function run_case(db_path, use_batch)
    ...
    local start_time = fetch_start(db_conn, "clip_right")
...
local function run_zero_gap(db_path)
    ...
    assert(db_conn:exec("UPDATE clips SET start_time = 2000 WHERE id = 'clip_right'"))
