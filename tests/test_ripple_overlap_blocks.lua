...
local function seed_db(db_path)
    ...
    local seed = string.format([[
        ...
        VALUES ('default_sequence', 'default_project', 'Timeline', 'sequence', 1000, 1, 48000, 1920, 1080, 0, 300, 0, %d, %d);
...
local function fetch_start(db_conn, id)
    local stmt = db_conn:prepare("SELECT sequence_start_frame FROM clips WHERE id = ?")
