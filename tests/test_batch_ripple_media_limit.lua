...
assert(clip.timeline_start.frames == 240, string.format("Ripple trim should keep clip anchored; expected 240, got %d", clip.timeline_start.frames))
assert(clip.source_in.frames == 0, string.format("source_in should clamp at 0; got %d", clip.source_in.frames))
assert(clip.duration.frames == 600, string.format("duration should extend only by available handle; expected 600, got %d", clip.duration.frames))
assert(clip.source_out.frames == 600, string.format("source_out should equal source_in + duration; expected 600, got %d", clip.source_out.frames))
