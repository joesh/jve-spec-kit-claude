...
--
-- Test media has 8 video frames. Clip occupies frames 0-5, gap at 6+.
--
...
-- Create TMB with a clip at frames 0-5, gap at frames 6-29.
-- Test media has 8 video frames — duration=6 stays well within bounds.
...
PLAYBACK.SET_BOUNDS(pc, 30, 24, 1)
...
PLAYBACK.SET_CLIP_WINDOW(pc, "video", 0, 30)
PLAYBACK.SET_CLIP_WINDOW(pc, "audio", 0, 30)

-- 1. Seek to frame 2 (inside clip) — should display a frame
PLAYBACK.SEEK(pc, 2)
local w1, h1 = EMP.SURFACE_FRAME_SIZE(test_surface)
check(w1 > 0 and h1 > 0,
    string.format("Seek to clip: frame size %dx%d (expected >0x>0)", w1, h1))

-- 2. Seek to frame 15 (gap — no clip) — should clear frame
PLAYBACK.SEEK(pc, 15)
local w2, h2 = EMP.SURFACE_FRAME_SIZE(test_surface)
check(w2 == 0 and h2 == 0,
    string.format("Seek to gap: frame size %dx%d (expected 0x0)", w2, h2))

-- 3. Seek back to frame 3 (inside clip) — should display a frame again
PLAYBACK.SEEK(pc, 3)
