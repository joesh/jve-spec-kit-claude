...
local function setup_db(path)
    ...
    db:exec(string.format([[
        ...
        VALUES ('sequence', 'project', 'Seq', 'timeline', 30.0, 48000, 1920, 1080, 0, 0, 0, 240);
        INSERT INTO sequences (id, project_id, name, kind, frame_rate, audio_sample_rate, width, height, timecode_start_frame, playhead_value, viewport_start_value, viewport_duration_frames_value)
        VALUES ('selection_sequence', 'project', 'Selection Seq', 'timeline', 30.0, 48000, 1920, 1080, 0, 0, 0, 240);
        INSERT INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
        VALUES ('track_v1', 'sequence', 'V1', 'VIDEO', 'video_frames', 30.0, 1, 1);
        INSERT INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
        VALUES ('track_v2', 'sequence', 'V2', 'VIDEO', 'video_frames', 30.0, 2, 1);
        INSERT INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
        VALUES ('track_v3', 'sequence', 'V3', 'VIDEO', 'video_frames', 30.0, 3, 1);
        INSERT INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
        VALUES ('track_v4', 'sequence', 'V4', 'VIDEO', 'video_frames', 30.0, 4, 1);
        INSERT INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
        VALUES ('track_v5', 'sequence', 'V5', 'VIDEO', 'video_frames', 30.0, 5, 1);
        INSERT INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
        VALUES ('selection_track_v1', 'selection_sequence', 'Selection V1', 'VIDEO', 'video_frames', 30.0, 1, 1);
    ]], now, now))
...
local function fetch_clip(db, clip_id)
    local stmt = db:prepare([[
        SELECT track_id, start_value, duration_value, source_in_value, source_out_value
        FROM clips WHERE id = ?
    ]])
    ...
    return {
        ...
        source_in_value = stmt:value(3),
        source_in = stmt:value(3),
        source_out_value = stmt:value(4),
        source_out = stmt:value(4)
    }
end

local function fetch_track_clips(db, track_id)
    local stmt = db:prepare([[SELECT start_value, duration_value, id FROM clips WHERE track_id = ? ORDER BY start_value]])
...
clip_c.start_value = 2500   -- overlaps tail of clip A
...
assert(state_a.start_value == 0, "clip A start should remain 0")
...
assert(state_c.start_value == 2500, "clip C start mismatch")
...
clip_d.start_value = 5000   -- sits between C and B
...
clip_d.start_value = 2000   -- spans [2000, 7000) covering end of A, all of C, start of B
...
assert(state_d.start_value == 2000, "clip D start should be 2000")
assert(state_d.duration == 5000, "clip D duration should remain 5000")

local stmt_a = db:prepare("SELECT duration_value FROM clips WHERE id = ?")
...
db:exec([[INSERT OR IGNORE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_v2', 'sequence', 'V2', 'VIDEO', 'video_frames', 30.0, 2, 1)]])

local mover_clip = Clip.create("Mover", "media_C")
mover_clip.track_id = "track_v2"
mover_clip.start_value = 2000
...
assert(moved_on_v1.start_value == 2000,
...
db:exec([[INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_nudge_v1', 'sequence', 'NV1', 'VIDEO', 'video_frames', 30.0, 12, 1);
          INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_nudge_v2', 'sequence', 'NV2', 'VIDEO', 'video_frames', 30.0, 13, 1);]])

local base_left = Clip.create("Base Left", "media_C")
base_left.track_id = "track_nudge_v1"
base_left.start_value = 1000
...
mover_for_nudge.start_value = 3500
...
assert(moved_after_nudge.start_value == 1000,
...
stmt = db:prepare([[
    ...
    WHERE track_id = 'track_v2' AND start_value = 5000
]])
stmt:exec(); stmt:next()
...
local clip_row = db:prepare([[SELECT media_id, source_in_value, source_out_value FROM clips WHERE id = ?]])
...
local stmt_split = db:prepare([[SELECT id, start_value, duration_value FROM clips WHERE track_id = 'track_v3' ORDER BY start_value]])
...
while stmt_split:next() do
    table.insert(rows, {
        ...
        start_value = stmt_split:value(1),
...
--     print("track_v3 clip", row.id, row.start_value, row.duration)
-- end
...
gap_downstream.start_value = 7000
...
local gap_start = gap_upstream.start_value + gap_upstream.duration
local gap_end = gap_downstream.start_value
...
assert(right_row.start_value == 3000, string.format("right fragment should start at 3000ms, got %d", right_row.start_value))
...
local original_downstream_start = gap_downstream.start_value
...
local downstream_stmt = db:prepare("SELECT start_value FROM clips WHERE id = ?")
...
assert(downstream_start >= gap_upstream.start_value + upstream_after.duration,
...
gap_extend_clip.start_value = 20000
...
gap_extend_down.start_value = 26000
...
assert(extended_downstream.start_value == 27500,
    string.format("downstream clip should shift right by gap extension (expected 27500, got %d)", extended_downstream.start_value))
...
assert(batch_downstream.start_value == 28700,
    string.format("downstream clip should shift right by batch gap extension (expected 28700, got %d)", batch_downstream.start_value))

print("✅ Batch gap ripple expands gap and shifts downstream clips")

print("Test 10: Track1 gap ripple matches UI scenario")
db:exec([[INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_v6', 'sequence', 'V6', 'VIDEO', 'video_frames', 30.0, 6, 1)]])
...
track_clip2.start_value = 5614567
...
local gap_delta = track_clip2.start_value - (track_clip1.start_value + track_clip1.duration)
...
assert(track_clip2_after.start_value == 5614567 + 1500,
    string.format("track gap ripple should shift downstream clip by delta (expected %d, got %d)", 5614567 + 1500, track_clip2_after.start_value))
...
local current_gap = clip2_after_expand.start_value - (clip1_after_expand.start_value + clip1_after_expand.duration)
...
do
    local expected_start = clip1_post_close.start_value + clip1_post_close.duration
    assert(math.abs(clip2_post_close.start_value - expected_start) <= 1,
        string.format("gap should be closed (expected %d, got %d)", expected_start, clip2_post_close.start_value))
end
...
assert(clip2_after_trim.start_value == clip2_post_close.start_value - trim_amount,
    string.format("downstream clip should shift left by trim amount (expected %d, got %d)", clip2_post_close.start_value - trim_amount, clip2_after_trim.start_value))
...
selection_upstream.start_value = 0
...
selection_downstream.start_value = 7000
...
local selection_gap = selection_downstream.start_value - (selection_upstream.start_value + selection_upstream.duration)
...
local expected_contact = upstream_after_close.start_value + upstream_after_close.duration
assert(downstream_after_close.start_value == expected_contact or
       downstream_after_close.start_value == expected_contact + 1,
    string.format("downstream clip should align with upstream clip (expected %d or %d, got %d)",
        expected_contact, expected_contact + 1, downstream_after_close.start_value))
...
db:exec([[INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_v8', 'sequence', 'V8', 'VIDEO', 'video_frames', 30.0, 8, 1);
          INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_v9', 'sequence', 'V9', 'VIDEO', 'video_frames', 30.0, 9, 1)]])
...
local top_left_end = top_left_after.start_value + top_left_after.duration
local bottom_left_end = bottom_left_after.start_value + bottom_left_after.duration

-- Clamp uses a single delta across both tracks: smallest gap wins, larger gap may leave residue.
assert(top_right_after.start_value == top_left_end,
    string.format("top track should butt after closure (expected %d, got %d)", top_left_end, top_right_after.start_value))
-- Bottom gap is larger than top gap; expect bottom to move by the same delta as top, possibly leaving residual gap.
assert(bottom_right_after.start_value >= bottom_left_end,
    string.format("bottom track should not overlap its upstream clip (expected >= %d, got %d)", bottom_left_end, bottom_right_after.start_value))
...
db:exec([[INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_v10', 'sequence', 'V10', 'VIDEO', 'video_frames', 30.0, 10, 1)]])
db:exec([[INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_v11', 'sequence', 'V11', 'VIDEO', 'video_frames', 30.0, 11, 1)]])
...
clamp_right.start_value = 8000  -- 4000ms gap
...
local original_right_start = clamp_right.start_value
local original_gap = clamp_right.start_value - (clamp_left.start_value + clamp_left.duration)
...
local clamp_gap = clamp_right_after.start_value - (clamp_left_after.start_value + clamp_left_after.duration)
assert(clamp_gap <= original_gap,
    string.format("gap closure should not exceed original (orig=%d, got %d)", original_gap, clamp_gap))

local actual_shift = original_right_start - clamp_right_after.start_value
...
mixed_right.start_value = 8000 -- 4000ms gap
...
local mixed_gap = mixed_right_after.start_value - (mixed_left_after.start_value + mixed_left_after.duration)
...
chain_mid.start_value = 8000 -- gap of 5000 after left
...
chain_right.start_value = 15000 -- gap of 5000 after mid
...
local gap_one = chain_mid_after.start_value - (chain_left_after.start_value + chain_left_after.duration)
local gap_two = chain_right_after.start_value - (chain_mid_after.start_value + chain_mid_after.duration)
...
db:exec([[INSERT OR REPLACE INTO tracks (id, sequence_id, name, track_type, timebase_type, timebase_rate, track_index, enabled)
          VALUES ('track_v12', 'sequence', 'V12', 'VIDEO', 'video_frames', 30.0, 12, 1)]])
...
overwrite_clip.start_value = 5000
...
local stmt_overwrite = db:prepare("SELECT start_value, duration_value FROM clips WHERE id = ?")
...
local start_value = tonumber(stmt_overwrite:value(0))
local duration_after = tonumber(stmt_overwrite:value(1))
assert(start_value == 5000, string.format("overwritten clip start should remain 5000, got %s", tostring(start_value)))
...
local stmt_verify = db:prepare("SELECT start_value FROM clips WHERE id = ?")
