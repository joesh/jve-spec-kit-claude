...
--   sequence_start=200, the resolved entry's outer sequence_start must
--   reflect the master-coord offset:
--     outer sequence_start = clip.sequence_start + (mr.sequence_start - clip.source_in)
--                          = 200 + (30 - 0) = 230
...
--
-- Catches the resolver bug that overrides translated sequence_start with
-- the OUTER clip's sequence_start, losing master-coord position info.
...
local e = entries[1]
assert(e.sequence_start == 230, string.format(
    "outer sequence_start must reflect mr.sequence_start translated through clip; "
    .. "expected 230, got %s", tostring(e.sequence_start)))
