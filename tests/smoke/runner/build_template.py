...
Runs JVE once, imports the DRP fixture, exits. Result lives at
...
def build(force: bool = False) -> Path:
    ...
    try:
        runner.start()
        # NewProject creates an empty .jvp at the supplied path. Then
        # ImportResolveDatabase ingests the DRP.
        path_lit = str(scratch_jvp).replace("'", "\\'")
        drp_lit = str(drp_path).replace("'", "\\'")
        runner.eval(
            f"require('core.commands.new_project').create_at_path('{path_lit}')")
        runner.open_project(scratch_jvp)
        runner.eval(
            "require('core.command_manager').execute('ImportResolveDatabase', "
            f"{{ drp_path = '{drp_lit}' }})")
        # Save before shutdown so the .jvp on disk has the imported content.
        runner.eval("require('core.commands.save_project').execute({})")
    finally:
        runner.shutdown()
