...
(see ``tests/smoke/MIGRATION_ANALYSIS.md`` entry for that file).
...
to re-pin from this origin):
    ...
    split    → tests/smoke/cases/test_keymap_cmd_b_blades_at_playhead.py
    enabled  → tests/smoke/cases/test_keymap_d_toggles_clip_enabled.py
    nudge    → tests/smoke/cases/test_keymap_nudge_selection.py
    undo/redo→ tests/smoke/cases/test_keymap_undo_redo.py
    trim     → tests/smoke/cases/test_keymap_cmd_shift_bracket_trim_head_tail.py
    boundary → tests/smoke/cases/test_*_clamps.py
