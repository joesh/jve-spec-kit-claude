...
local gap2 = ts.get_tab_strip():clip_by_id(gap2_id)
...
local merged_gap = ts.get_tab_strip():clip_by_id(merged_gap_id)
...
local old_gap2 = ts.get_tab_strip():clip_by_id(gap2_id)
...
ts.set_selection({ts.get_tab_strip():clip_by_id(layout.clips.v1_right.id)})
...
local gap2_after_nudge = ts.get_tab_strip():clip_by_id(layout:gap_id("v1", 1000))
