...
local timeline_state = {
    playhead_position = Rational.new(10, 30, 1),
    clips = {},
}
...
function mock_pc.get_position()
    if mock_pc.timeline_mode then
        return timeline_state.playhead_position.frames
    end
    return mock_pc._position
end
function mock_pc.set_position(v)
    if mock_pc.timeline_mode then
        local rat = Rational.new(math.floor(v), mock_pc.fps_num, mock_pc.fps_den)
        timeline_state.set_playhead_position(rat)
        mock_pc._position = v
    else
        mock_pc._position = v
...
timeline_state.playhead_position = Rational.new(10, 30, 1)
...
assert(timeline_state.playhead_position.frames == 11,
    string.format("Expected frame 11, got %d", timeline_state.playhead_position.frames))
...
assert(timeline_state.playhead_position.frames == 9,
    string.format("Expected frame 9, got %d", timeline_state.playhead_position.frames))
...
assert(timeline_state.playhead_position.frames == 0,
    string.format("Expected frame 0, got %d", timeline_state.playhead_position.frames))
...
assert(timeline_state.playhead_position.frames == 40,
    string.format("Expected frame 40, got %d", timeline_state.playhead_position.frames))
