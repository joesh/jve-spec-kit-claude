...
local function init_database(path)
    ...
    assert(db:exec([[
        ...
        VALUES ('media_stub', 'default_project', 'Stub', '/tmp/stub.mov', 1000, 30.0, 1920, 1080, 2, 'prores', strftime('%s','now'), strftime('%s','now'), '{}');
...
local TEST_DB = "/tmp/test_split_clip_mutations.db"
...
while stmt:next() do
    ...
    assert(start_time == 0, "Original clip start_time should be 0 after undo")
    assert(duration == 1000, "Original clip duration should be restored after undo")
end
stmt:finalize()
