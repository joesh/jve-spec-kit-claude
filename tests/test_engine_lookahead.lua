...
print("\n--- no re-pre-buffer for same audio clip ---")
do
    local engine = make_engine()
    reset_logs()
    ...
    engine:shuttle(1)
    pump_tick() -- tick 1: advances to 91, pre-buffers clip_b audio
    ...
    engine:stop()
    clear_timers()
    print("  no re-pre-buffer for same audio clip passed")
end
...
--------------------------------------------------------------------------------
-- Test 5: Pre-buffer resets on clip change
--------------------------------------------------------------------------------
...
do
    ...
    engine:shuttle(1)
    pump_tick() -- tick 1: frame 99, within threshold -> pre-buffers clip_b audio

    local audio_count_before = #audio_pre_buffer_calls
    assert(audio_count_before > 0, "Should pre-buffer clip_b audio near boundary")

    pump_tick() -- tick 2: frame 100 -> now IN clip_b

    -- After entering clip_b, approaching clip_b's end at frame 200
    -- Distance = 100 frames = 4.17s, outside 2s audio threshold -> no pre-buffer
    assert(#audio_pre_buffer_calls == audio_count_before, string.format(
