...
local seed = string.format([[
    ...
    VALUES
        ...
        ('clip_right', 'default_project', 'timeline', 'Right', 'track_v2', 'media1', 'default_sequence',
         15000, 1000, 0, 1000, 1000, 1, 1, 0, %d, %d);
...
cmd:set_parameter("edge_infos", {
    {clip_id = "clip_right", edge_type = "gap_before", track_id = "track_v2"}
})
cmd:set_parameter("delta_frames", -20000)

local result = command_manager.execute(cmd)
assert(result.success, result.error_message or "BatchRippleEdit should clamp instead of overlapping")

local stmt = assert(db:prepare("SELECT timeline_start_frame FROM clips WHERE id = 'clip_b'"))
assert(stmt:exec() and stmt:next(), "clip_right missing after ripple")
local start_value = tonumber(stmt:value(0))
stmt:finalize()

assert(start_value == 2000, string.format("clip_b should clamp to neighbor end (expected 2000, got %s)", tostring(start_value)))
