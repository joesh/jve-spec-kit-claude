...
-- Snapshot all (id, file_path) rows in the current project.
...
local function snapshot_media()
    ...
    local stmt = assert(db:prepare(
        "SELECT id, file_path FROM media ORDER BY file_path"))
    assert(stmt:exec())
    local rows_by_path = {}
    while stmt:next() do
...
print(string.format("  %d media rows", media_count))

-- ---------------------------------------------------------------------------
-- Step 2: Re-import the same DRP into the same destination. Capture
-- media rows again and assert id-stability per file_path. media.id IS the
-- DRP MediaRef DbId (deterministic from the DRP bytes), so a fresh import
-- into a fresh project MUST reproduce the same id for the same physical
-- file. Otherwise peak_cache and any other per-media-id cache orphans on
-- every re-convert.
-- ---------------------------------------------------------------------------
print("\n--- Step 2: re-import + verify stable ids ---")
