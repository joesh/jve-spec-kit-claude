...
db:exec(string.format(
    "INSERT INTO projects (id, name, fps_mismatch_policy, settings, created_at, modified_at) VALUES ('proj1', 'Quickstart', 'resample', '{\"master_clock_hz\":192000,\"default_fps\":{\"num\":24,\"den\":1}}', %d, %d)",
    now, now))
...
-- V13 placeholder master sequence (was V8 NULL media_id) — created once.
db:exec([[
...
for i, name in ipairs({"Scene01_v1", "Scene02_v1", "Scene03_v2"}) do
    ...
    stmt:bind_value(3, (i - 1) * 100)
    assert(stmt:exec(), "clip insert failed: " .. tostring(stmt:last_error()))
    stmt:finalize()
end

local verify = db:prepare("SELECT name FROM clips WHERE id = 'rc1'")
assert(verify:exec()) verify:next()
