...
-- Originally discovered via debug scripts — NULL sequence_start caused downstream
-- arithmetic failures.
--
-- Two defense layers:
...
-- Schema rejects NULL sequence_start_frame
local ok_ts = db:exec([[
...
VALUES ('_v13_placeholder_media', 'proj1', 'placeholder', '_placeholder', 10800000, 25, 1, 1920, 1080, 0, 'raw', 0, 0);
INSERT INTO sequences (id, project_id, name, kind, fps_numerator, fps_denominator, audio_sample_rate, width, height, created_at, modified_at)
VALUES ('_v13_placeholder_master', 'proj1', 'placeholder_master', 'master', 25, 1, NULL, 1920, 1080, 0, 0);
INSERT INTO tracks (id, sequence_id, name, track_type, track_index, enabled, locked, muted, soloed, volume, pan)
...
VALUES ('_v13_placeholder_mr', 'proj1', '_v13_placeholder_master', '_v13_placeholder_track', '_v13_placeholder_media', 0, 10800000, 0, 10800000, 48000, 1, 1.0, 0, 0, 0);
...
check("schema rejects NULL sequence_start", ok_ts ~= 0)
...
check("roundtrip sequence_start=89849", clips[1].sequence_start == 89849)
...
check("roundtrip frame_rate.fps_numerator=25", clips[1].frame_rate.fps_numerator == 25)
check("roundtrip frame_rate.fps_denominator=1", clips[1].frame_rate.fps_denominator == 1)
db:exec("DELETE FROM clips")

-- ── Boundary: negative sequence_start (valid — pre-roll) ──
db:exec([[
    INSERT INTO clips (id, project_id, name, track_id, sequence_id, owner_sequence_id, sequence_start_frame, duration_frames, source_in_frame, source_out_frame, enabled, created_at, modified_at, master_layer_track_id, master_audio_track_id, fps_mismatch_policy, volume, playhead_frame) VALUES
...
check("negative sequence_start loads", #clips == 1)
check("negative sequence_start=-100", clips[1].sequence_start == -100)
db:exec("DELETE FROM clips")
...
check("large sequence_start=8640000", clips[1].sequence_start == 8640000)
