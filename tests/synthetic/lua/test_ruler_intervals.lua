...
do
    ...
    local interval_frames2, hint2, _ = frame_utils.get_ruler_interval(viewport_frames, rate24, 100, pixels_per_frame)
    assert(hint2 == "seconds", "expected seconds hint for mid intervals")
    assert_eq(interval_frames2, 24, "seconds bucket should be 1s (24 frames)")
end
...
do
    ...
    local _, hint3, val3 = frame_utils.get_ruler_interval(24 * 4, rate24, 100, pixels_per_frame)
    assert(hint3 == "seconds", "expected seconds hint for mid zoom")
    assert(math.abs(val3 - 1.0) < 0.01, "should round up to 1-second ticks instead of half-seconds")
end
...
do
    ...
    local pixels_per_ms = width_px / viewport_ms
    local pixels_per_frame = pixels_per_ms * (1000/24)
    local _, hint4, val4 = frame_utils.get_ruler_interval(24 * 9, rate24, 100, pixels_per_frame)
    assert(hint4 == "seconds", "expected seconds hint for ~10s viewport")
    assert(math.abs(val4 - 1.0) < 0.01, "should prefer 1-second ruler steps at this zoom")
end
...
do
    ...
    local pixels_per_ms = width_px / viewport_ms
    local pixels_per_frame = pixels_per_ms * (1000/24)
    local _, hint5, val5 = frame_utils.get_ruler_interval(24 * 3, rate24, 100, pixels_per_frame)
    assert(hint5 == "seconds", "expected seconds hint for ~3s viewport")
    assert(math.abs(val5 - 1.0) < 0.01, "should elevate to 1-second ticks instead of half-seconds")
end

-- Very large targets should yield minute buckets
do
    local _, hint6, val6 = frame_utils.get_ruler_interval(24 * 600, rate24, 500000, 1)
    assert(hint6 == "minutes", "expected minutes hint for huge intervals")
    assert(val6 > 0.5, "minute interval value should be reasonable")
end
