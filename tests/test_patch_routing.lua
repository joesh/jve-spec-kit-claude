...
--   2. After SetPatch(type=VIDEO, src=2, rec=3), find_by_record(seq, VIDEO, 3)
--      returns source_track_index=2.
--   3. After the above, find_by_record(seq, VIDEO, 2) returns nil.
--   4. Re-routing: patching src=2 to a new rec causes old rec row to become nil.
--   5. Cross-type SetPatch (track_type VIDEO but used as AUDIO) — VIDEO src
--      and AUDIO src at same index are independent slots (no cross-type confusion).
...
local result = Patch.find_by_record("seq", "VIDEO", 1)
assert(result == nil,
...
local p2 = Patch.find_by_record("seq", "VIDEO", 3)
assert(p2, "find_by_record(seq, VIDEO, 3) must return a patch")
assert(p2.source_track_index == 2,
    "expected source_track_index=2, got " .. tostring(p2.source_track_index))
print("  find_by_record(seq, VIDEO, 3).source_track_index = 2 — OK")

-- ── Test 3: find_by_record(seq, VIDEO, 2) is nil ─────────────────────────
print("-- 3: find_by_record nil for unrelated record row --")
local p3 = Patch.find_by_record("seq", "VIDEO", 2)
assert(p3 == nil,
    "find_by_record(seq, VIDEO, 2) must be nil — only rec=3 was patched")
print("  find_by_record(seq, VIDEO, 2) is nil — OK")
...
local p4a = Patch.find_by_record("seq", "VIDEO", 1)
assert(p4a and p4a.source_track_index == 2,
    "after re-route, find_by_record(seq, VIDEO, 1) must return source=2")
local p4b = Patch.find_by_record("seq", "VIDEO", 3)
assert(p4b == nil,
    "after re-route, find_by_record(seq, VIDEO, 3) must return nil")
...
local pa = Patch.find_by_record("seq", "AUDIO", 2)
