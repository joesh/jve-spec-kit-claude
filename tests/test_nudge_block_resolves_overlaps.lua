...
-- Regression: nudging a block of clips right must resolve occlusions on the same track (no VIDEO_OVERLAP).
...
local DB_PATH = "/tmp/jve/test_nudge_block_resolves_overlaps.db"
os.remove(DB_PATH)
...
do
    assert(db:exec("INSERT INTO sequences(id,project_id,name,kind,fps_numerator,fps_denominator,audio_sample_rate,width,height,view_start_frame,view_duration_frames,playhead_frame,created_at,modified_at) VALUES('_v13_placeholder_master','proj','PlaceholderMaster','master',24,1,NULL,1920,1080,0,2000,0,0,0);"))
...
-- Three clips on the same track; moving the first two right will overlap the third unless occlusions are resolved.
...
-- Verify there are no overlaps on v1
local q = db:prepare("SELECT id, sequence_start_frame, duration_frames FROM clips WHERE track_id = 'v1' ORDER BY sequence_start_frame")
...
os.remove(DB_PATH)
print("✅ Nudge block resolves overlaps instead of hitting VIDEO_OVERLAP")
