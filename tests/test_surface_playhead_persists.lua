...
-- (post-022/H1) reads ALL per-sequence view-state from displayed_tab.cache.
-- Minimal stub carries everything viewport_state asks for.
local stub_cache = nil
local function install_stub_strip(seq_id, viewport_start, viewport_duration, playhead)
    stub_cache = {
...
local function reset(viewport_start, viewport_duration, playhead)
    ...
    install_stub_strip("test_seq", viewport_start, viewport_duration, playhead)
end
...
do
    ...
    assert(stub_cache.viewport_start_time == 7500,
...
do
    ...
    local persist_calls = 0
    viewport_state.surface_playhead(function() persist_calls = persist_calls + 1 end)
    assert(stub_cache.viewport_start_time == 0,
