...
local result = drp._decode_bt_clip_path(video_hex)
...
result = drp._decode_bt_clip_path(embedded_audio_hex)
...
result = drp._decode_bt_clip_path(standalone_audio_hex)
...
assert(drp._decode_bt_clip_path(nil) == nil, "nil → nil")
assert(drp._decode_bt_clip_path("") == nil, "empty → nil")
assert(drp._decode_bt_clip_path("0000") == nil, "short → nil")
...
assert(drp._decode_bt_clip_path(garbage) == nil, "garbage → nil")
...
local synthetic_video = "0000000200000019812800000000000000001400" ..
    ...
    "1205" .. -- F2 tag + len=5
    "612e6d6f76" -- "a.mov"
result = drp._decode_bt_clip_path(synthetic_video)
...
local synthetic_audio = "000000020000001781280000000000000000" ..
    ...
    "1205" .. -- F2 tag + len=5
    "612e6d6f76" -- "a.mov"
result = drp._decode_bt_clip_path(synthetic_audio)
assert(result == "/tmp/a.mov", string.format(
...
assert(drp._decode_bt_clip_path("0000000200000019812800000000000000001400" ..
...
assert(drp._decode_bt_clip_path(truncated) == nil, "truncated blob → nil")
...
local empty_dir = "0000000200000015812800000000000000001400" ..
    ...
    "1205" .. -- F2 tag + len=5
    "612e6d6f76" -- "a.mov"
assert(drp._decode_bt_clip_path(empty_dir) == nil, "empty directory → nil")
...
assert(drp._decode_bt_clip_path(empty_fname) == nil, "empty filename → nil")
...
assert(drp._decode_bt_clip_path(runaway) == nil, "runaway varint → nil")
