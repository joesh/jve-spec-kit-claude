...
A new smoke is a `tests/live/cases/test_*.py` file subclassing
`tests/live/runner/case.JVESmokeCase`. The runner is a
...
`tests/live/cases/test_*_clamps.py` and
...
| `package.loaded["ui.panel_manager"] = { ... }` from inside `--test` script | Mocks production code. We test the actual app. | Drive via smoke. If you find yourself wanting a mock, the test belongs in `tests/live/`. |
...
python3 -m unittest tests.live.cases.test_X -v
