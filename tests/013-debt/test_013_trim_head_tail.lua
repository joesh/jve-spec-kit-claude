...
-- timeline_start moves forward by N, duration shrinks by N, source_in
-- advances by the policy-appropriate source delta, source_out unchanged.
-- TrimTail is the mirror: timeline_start unchanged, duration shrinks by
...
do
    ...
    assert(c.timeline_start == 110 and c.duration == 90 and
           c.source_in == 10 and c.source_out == 100,
        string.format("expected [tl=110,d=90,s=(10,100)]; got [tl=%d,d=%d,s=(%d,%d)]",
            c.timeline_start, c.duration, c.source_in, c.source_out))
    print("  ok")
end
...
do
    ...
    local c = load_clip(db, "c")
    assert(c.timeline_start == 100 and c.duration == 90 and
...
do
    ...
    local c = load_clip(db, "c")
    assert(c.timeline_start == 225 and c.duration == 75 and
...
do
    ...
    local c = load_clip(db, "c")
    assert(c.timeline_start == 24 and c.duration == 72, string.format(
...
do
    ...
    local c = load_clip(db, "c")
    assert(c.timeline_start == 100 and c.duration == 10
