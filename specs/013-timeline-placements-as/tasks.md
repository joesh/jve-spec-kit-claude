...
- [x] **T029a [P]** INV-5 audio-channel-out-of-bounds at resolve time — combined with T013a in `tests/test_inv5_channel_index_bounds.lua`. Resolver scans each clip's `clip_channel_override` rows during `resolve_nested` and asserts `channel_index < count_master_audio_channels(nested)`; assert message names the clip id + bad channel_index + master id + actual channel count. (No silent fallback to master state — explicit per the data-model.md INV-5 contract.)
