...
- [x] **T056j partial [P]** `src/lua/core/commands/collapse_audio.lua` — forward path + comprehensive refusals. Refusals (rule 1.14): empty selection, sequence_id mismatch, non-audio clip, already-composite clip, different nested across selection, divergent source/timeline windows, different fps_mismatch_policy, not-all-in-one-link-group, duplicate master_audio_track_id. Forward: capture each selected clip's full V13 state; DELETE selected clips (cascade overrides + link entries); INSERT composite on topmost selected track (lowest track_index) with `master_audio_track_id=NULL`; project per-channel state — unselected nested tracks → enabled=0 disables on composite at ch = track_index-1; per-clip overrides → composite at ch = track_index-1; non-unity per-clip volume → per-channel gain on composite. Re-link via `ClipLink.add_to_group`. **Undo deferred** (T056g/CT-C21d roundtrip exercises Expand→Collapse→ExpandAudio.undo path which already works for this case).
