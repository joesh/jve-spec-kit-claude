...
- [x] **T013a [P]** INV-5 — `clip_channel_override.channel_index` that points past the nested sequence's current audio channel count is rejected at resolve time with a loud assert naming the clip + channel_index. Combined with T029a in `tests/test_inv5_channel_index_bounds.lua`: write-time refusals via ToggleClipChannel/SetClipChannelGain + resolve-time assert via `Sequence:pick_in_range` enforced inline in pick_nested. First-landing scope: enforces only when clip directly references kind='master' (deferred for nested-of-nested).
...
- [x] **T018 [P]** CT-R1: master resolution (leaf) returns one `ResolvedEntry` per media_ref with correct `media_path` and provenance length 1. Path: `tests/test_pick_master_leaf.lua`.
- [x] **T019 [P]** CT-R2: one-level nested resolution through a clip; provenance length 2; `timeline_start` translated through the clip's window. Path: `tests/test_pick_nested_one_level.lua`.
- [x] **T020 [P]** CT-R3: three-level chain (nested → nested → master); provenance length 3. Path: `tests/test_pick_nested_deep.lua`.
...
- [x] **T029a [P]** INV-5 audio-channel-out-of-bounds at resolve time — combined with T013a in `tests/test_inv5_channel_index_bounds.lua`. Resolver scans each clip's `clip_channel_override` rows during `pick_nested` and asserts `channel_index < count_master_audio_channels(nested)`; assert message names the clip id + bad channel_index + master id + actual channel count. (No silent fallback to master state — explicit per the data-model.md INV-5 contract.)
...
- [x] **T030** Implement `Sequence:pick_in_range(seq_id, start_frame, end_frame, context)` in `src/lua/models/sequence.lua` per `contracts/resolver.md`. **Rule 2.5/2.6: `pick_in_range` is the orchestrator; each responsibility lives in a named helper ≤ 30 lines.** Required decomposition:
  - `pick_master_leaf(seq_id, range, context)` — iterate `media_refs`, clip to range, emit leaf entries with `media_id → media.file_path`, apply `media_refs_channel_state`.
  - `pick_nested_clips(seq_id, range, context)` — iterate overlapping `clips`, recurse per clip.
...
- [x] **T052 [P]** FR-020 coverage: five rapid ToggleClipChannel.execute calls produce five DISTINCT undo captures, each independently reversible (no shared state, no coalescing). Same-channel rapid toggles also produce 5 distinct captures. Path: `tests/test_override_undo_granularity.lua`. **Note**: command_manager-level persistence-grouping (the `commands` table rows produced one-per-execute) is the responsibility of command_manager itself; that end-to-end check is blocked on `database.load_clips` migration from V8 (clip_kind/media_id/master_clip_id) to V13 — unrelated cleanup task. The command-layer mechanism that command_manager builds atop is what this test pins.
- [x] **T052a [P]** Insert/Overwrite atomic-undo (multi-row commands): single Insert.undo reverses V + N A clips + link_group atomically. Tested for both composite (1 V + 1 A) and expanded audio mode (1 V + 3 A on a 3-A-track master, with 2 auto-created owner tracks). Path: `tests/test_insert_overwrite_atomic_undo.lua`. Overwrite atomic-undo coverage (restoring trimmed/removed clips + removing new clip) is a follow-up — Overwrite.undo is implemented but exercised under the existing CT-C2 contract test rather than the dedicated atomic-undo guard here.
...
- [x] **T056a [P]** CT-C20 ExpandAudio happy path — given V + composite A clip referencing a 4-audio-track master on a target sequence with only 1 A track, ExpandAudio replaces the source A clip with 4 A clips on tracks A1..A4 (auto-creating A2..A4), each with a distinct non-NULL `master_audio_track_id`; the V clip's link group now contains V + 4 A clips; per-channel overrides on the source clip project onto the corresponding expanded clip. Path: `tests/test_expand_audio.lua`.
- [x] **T056b [P]** CT-C20b ExpandAudio refusal cases — refuse on 1-audio-track master ("nothing to expand"); refuse on already-expanded clip (non-NULL `master_audio_track_id`); refuse on collision with an existing clip on A2..A4 in the source's time range (named-offender error); refuse on sequence_id mismatch (rule 2.29); refuse on video clip (only audio expandable). No mutation in any refusal case. Path: `tests/test_expand_audio_refusals.lua`.
- [x] **T056c [P]** CT-C20c ExpandAudio undo — single undo restores the source composite clip + its overrides + its link_group membership and removes the N expanded clips and any auto-created tracks. Order: DELETE expanded clips (cascade clip_links) → DELETE auto-created tracks → Clip.restore_v13_state(source). Path: `tests/test_expand_audio_undo.lua`.
- [x] **T056d [P]** CT-C21 CollapseAudio happy path — given V + 4 expanded A clips, CollapseAudio on all 4 produces V + 1 composite A clip on the topmost source track with `master_audio_track_id=NULL`; per-channel state preserved. Path: `tests/test_collapse_audio.lua`.
- [x] **T056e [P]** CT-C21b CollapseAudio partial selection — given V + 4 expanded A clips (A1..A4), CollapseAudio on {A1, A2} produces V + composite-on-A1 (with per-channel disables on channels 2 and 3 — A3 + A4 unselected) + A3 + A4 untouched. Audibly identical to pre-collapse. Path: `tests/test_collapse_audio_partial.lua`.
- [x] **T056f [P]** CT-C21c CollapseAudio refusal cases — covers empty selection, sequence_id mismatch, already-composite clip, different masters across selection, divergent source windows, not-all-in-one-link-group, duplicate master_audio_track_id. No mutation in any refusal case. Path: `tests/test_collapse_audio_refusals.lua`.
- [x] **T056g [P]** CT-C21d Expand→delete→Collapse→Expand roundtrip — given V + 4 expanded A clips, user deletes A2, CollapseAudio on remaining {A1, A3, A4} yields composite with disable on channel 1 (A2's channel); subsequent ExpandAudio on the composite re-creates 4 A clips with the A2-track clip's ch=0 still disabled (silent); ClearClipOverride on that ch=0 restores audibility. Lossless audible-state roundtrip. Path: `tests/test_expand_collapse_roundtrip.lua`.
- [x] **T056h [P]** CT-C1b (regression on AddClipsToSequence/Insert/Overwrite — FR-002 + FR-025): with `audio_drop_mode='expanded'` arg, Insert of a 4-A-track master onto a 1-A-track sequence emits 1 V + 4 A rows with distinct non-NULL `master_audio_track_id` values, auto-creates 3 audio tracks, all 5 rows in one link_group; refused on collision. Default (no arg) is composite. Path: `tests/test_insert_audio_drop_mode.lua`.

### 3.5.c.b — Implementation

- [x] **T056i [P]** `src/lua/core/commands/expand_audio.lua` — full forward + undo. Refusals (rule 1.14 fail-fast): track-type, sequence_id mismatch, already-expanded, < 2 nested A tracks, collision with named offender. Forward: auto-create owner A tracks at matching `track_index`; INSERT one expanded clip per nested A track with distinct `master_audio_track_id`; rewrite link_group membership (extends source's via new `ClipLink.add_to_group`, or creates a new group when source had none); project per-channel overrides from `Clip.capture_v13_state.overrides` onto expanded clips (1-channel-per-track masters: source ch=N → expanded[track_index=N+1] ch=0). Undo: DELETE expanded clips → DELETE auto-created tracks → `Clip.restore_v13_state(source)`.
- [x] **T056j [P]** `src/lua/core/commands/collapse_audio.lua` — full forward + undo. Forward: comprehensive pre-flight refusals; capture each selected clip's full V13 state; DELETE selected clips; INSERT composite on topmost selected track; project per-channel state (unselected → disables; per-clip overrides → composite at ch=track_index-1; non-unity volume → gain); re-link composite. Undo: DELETE composite → `Clip.restore_v13_state` for each captured selected clip. Snapshot test (`test_collapse_audio_undo`) verifies byte-for-byte restoration. Required fix to `Clip.restore_v13_state` to also restore `master_audio_track_id` (was missing — would have NULL'd the column on every restored expanded clip).
- [x] **T056k** Updated `_place_shared` (the shared scaffolding for Insert/Overwrite — and by extension AddClipsToSequence which routes through the same placement plan) to accept optional `audio_drop_mode ∈ {'composite','expanded'}` arg defaulting to composite. Expanded mode: enumerates the nested's A tracks, auto-creates owner A tracks at matching `track_index` if missing, emits one A clip per nested track with distinct `master_audio_track_id`, refuses on collision (rule 1.14 — names the offending clip id and range). All N A clips share the V's link_group. `Clip._create_v13_row` now binds `master_audio_track_id` (column 12) so the new field round-trips through INSERT. Existing CT-C1 green; new T056h covers the expanded path + collision refusal + unknown-mode refusal.
- [x] **T056l** Extend `Sequence:pick_in_range` to apply `clip.master_audio_track_id` as an audio-track filter symmetric to `master_layer_track_id` (resolver.md track-selector step). Defense-in-depth: dangling non-NULL value asserts loudly per G-R5. Drives T021a green.
...
- [x] **T059 [P]** CT-C15 SetSequenceStartTC — propagates to timeline-position translation for all referencing clips. Path: `tests/test_set_sequence_start_tc.lua`.
- [x] **T059a [P]** FR-017 default-derivation coverage: master path tested in `tests/test_sequence_start_tc_defaults.lua` — `Sequence.ensure_master` populates `video_start_tc_frame` from the media's TC origin and `audio_start_tc_samples` from the same TC at the audio rate; video-only master leaves `audio_start_tc_samples` NULL; TC=0 is honored as a real value. **Non-master derive-from-first-clip is a future addition** (sequences created via `Sequence.create` accept the columns explicitly and don't auto-derive — adding auto-derivation is non-trivial and orthogonal to first-landing).
- [x] **T060 partial [P]** CT-C16 SetFpsMismatchPolicy — project + sequence scopes covered; clip-scope structural mutation is deferred (re-compute duration_frames + ripple + flip linked V+A together is a non-trivial follow-up). The deferral is loud-failed with a TODO message and tested. Path: `tests/test_set_fps_mismatch_policy.lua`. **Follow-up**: implement clip-scope structural mutation (T064 partial — see `set_fps_mismatch_policy.lua` execute_clip stub).
- [x] **T060a [P]** Scenario-7 owning command coverage (FR-007): GrowMasterMedium contract test in `tests/test_grow_master_medium.lua` — first-landing scope is medium='audio' add (the dominant Scenario-7 case: sync audio to a previously video-only master). 3 existing V clips on edit gain linked A companions; clips already with an A companion in their link_group are not duplicated; non-master sequence_id refused; missing track_spec.media_id refused. Adding video to an audio-only master + multi-channel A track creation are follow-ups.
...
- [x] **T063 [P]** Create `src/lua/core/commands/set_sequence_start_tc.lua` — Args `{ sequence_id, medium ∈ {'video','audio'}, tc_value }` (rule 2.29 — `sequence_id` required).
- [x] **T064 partial [P]** Created `src/lua/core/commands/set_fps_mismatch_policy.lua`. Project + sequence scopes complete. Clip-scope structural mutation (re-compute duration_frames + ripple downstream + flip linked V+A together) is loud-failed with a follow-up message. Original task body retained for the follow-up: --
...
- [x] **T064a partial [P]** Created `src/lua/core/commands/grow_master_medium.lua` (Scenario-7 owning command) — first-landing scope is medium='audio' add. Mutations: (1) INSERT new master A track + media_ref; (2) for every video clip referencing the master that lacks an audio companion in its link_group, INSERT a new A clip on the parent's first A track and link the pair (creating a new link_group if neither side had one); (3) emit sequence_content_changed on master + every parent touched. Undo not yet implemented; medium='video' add deferred. Refused if media has no audio channels, parent has no A track, or sequence is non-master.
...
- [ ] **T093** Update `src/lua/core/playback/playback_engine.lua` — replace every `clip.media_id` / `clip.clip_kind` / `clip.master_clip_id` / `clip.offline` read with the resolver-driven chain (via `get_video_in_range`/`get_audio_in_range` wrappers from T031, or `pick_in_range` directly if the caller wants unfiltered entries). No shim to preserve old calling patterns; rewrite the consumer to the new entry shape. T091a grep-assertion gates the file.
- [ ] **T094** Create `src/lua/core/export/export_engine.lua` — shares `pick_in_range` with playback (FR-019); export-only policies (codec, bit-depth, colorspace, proxy-vs-source, resample filter) applied in the pipeline ABOVE the resolver, never inside it.
...
```
Task: "CT-R1 master leaf resolution in tests/test_pick_master_leaf.lua"
Task: "CT-R2 one-level nested resolution in tests/test_pick_nested_one_level.lua"
...
