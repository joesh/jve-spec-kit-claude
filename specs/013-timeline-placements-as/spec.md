...
- **FR-011a (full-pool import)**: Importers MUST materialize a master sequence for **every** media-pool item the source project carries (every clip in every bin), not only the media placed on a timeline — so the entire pool is browseable and openable, mirroring the source app's pool. A pool item whose source project carries no timecode origin (which the importer must not probe for, per the importers-do-not-probe rule) MUST still import as a relinkable media item; its master sequence is deferred (built when the media is later relinked or probed) **only while the item is pool-only**. When such a no-timecode item is also placed on a timeline, its master MUST be built immediately at timecode origin 00:00:00:00 — the universal NLE convention for media that carries no source timecode — spanning [0, duration]; the relink/probe timecode-sync path shifts the origin if a real timecode later appears. (Master creation must not crash on absent timecode; zero is the defined origin, not a fallback masking missing data.) Master creation is idempotent: a pool item that is also placed on a timeline yields exactly one master.
- **FR-011b (deleted-master regeneration)**: A timeline clip whose media-pool master is **absent** — the source app permits a dangling reference (e.g. Resolve clears a clip's pool link, leaving only the inline file path) — MUST still be placed, not dropped: "import everything in the source project" applies to placed clips, not only pool items. The importer MUST regenerate a relinkable master from the clip's **surviving inline fields** (file path, frame rate, and timecode origin where present), never by probing the file (importers-do-not-probe). The regenerated master's duration is the **used-span lower bound** (the longest source extent across the clip's placements); relink later replaces it with the real file length. For audio whose deleted master carried the sample rate, the importer MUST stand in the project's audio rate (the same value the project default derives) so the clip places and the master is valid; relink probes the real file and replaces the rate, rescaling coordinates. When the surviving file path is **ambiguous** (it resolves to more than one candidate pool master), the importer MUST leave the clip offline for the user to reconnect with normal relink tools rather than guess — the editor surfaces this as a reconnect affordance (Resolve-style indicator).
