...
7. **Writes the master row's `playhead_position` via `core.playhead.set(clip.sequence_id, target_frame)`** where `target_frame = pick_playhead_target(opts, clip)` — caller-supplied `opts.playhead_frame` wins (else default = `clip.source_in`), then the value is **clamped to the clip's source range** `[min(source_in, source_out), max(source_in, source_out)]` so the parking frame never lies outside the viewer's mark window (FR-024 v2 parking-clamp; the post-load playhead is free per FR-016a). Single canonical model write — `transport`'s `playhead_changed` listener picks up the signal and seeks the source engine bound in step 6. No view/model drift, no double-seek.
