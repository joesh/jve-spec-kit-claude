...
7. **Writes the master row's `playhead_position` via `core.playhead.set(clip.sequence_id, target_frame)`** where `target_frame = pick_playhead_target(opts, clip)` — caller-supplied `opts.playhead_frame` wins (else default = `clip.source_in`), then **clamped to `[source_in, source_out]`** so the parking frame never lies outside the viewer's mark window (FR-024 v2; post-load is free per FR-016a). Forward clips only — `effective_source` rejects reverse clips upstream. Single canonical model write — `transport`'s `playhead_changed` listener picks up the signal and seeks the source engine bound in step 6. No view/model drift, no double-seek.
