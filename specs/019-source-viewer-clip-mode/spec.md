...
- **FR-003** `source_viewer.load_clip(clip_id, opts)` — enters live-bound mode. Loads the clip row + its source sequence (`clip.sequence_id`); binds playback by calling `SequenceMonitor:load_sequence(clip.sequence_id)` (same code path as staged mode); **parks the source-side playhead by writing `master.playhead_position` via `core.playhead.set(clip.sequence_id, target_frame)`** — `transport`'s `playhead_changed` listener then seeks the source engine bound above, so the src tab's ruler (reads `master.playhead_position`) and the source viewer's engine end up at the same frame with one canonical model write. `target_frame = opts.playhead_frame` when supplied; otherwise `clip.source_in`. The Shift+F dispatcher (`OpenClipInSourceMonitor`, FR-024 v2 2026-05-22) supplies `opts.playhead_frame = Clip.owner_frame_to_source(clip, rec_playhead)` so the source tab + viewer land on the source frame currently under the record playhead — same behavior pattern as `F`/MatchFrame. Stashes `clip_id` so the mark-setter and selection_hub publish know to read from clip columns. Transitions the mode flag to `"live_bound_clip"`.
...
- **FR-016d** In live-bound mode, `effective_source.get()` returns `(sequence_id=clip.sequence_id, in=clip.source_in_frame, out=clip.source_out_frame)` — pass-through to the clip's underlying source sequence with the clip's own source range as in/out. Insert/Overwrite from the record-side use these values; because the same `source_in`/`source_out` columns drive both the retrim (FR-013/014) and the effective-source pass-through, the source-viewer marks and the edit-into-timeline bounds stay in lockstep automatically with no extra propagation logic. Implementation: source viewer's live-bound state contributes through the existing `effective_source._source_viewer_seq_id` channel, extended to carry an optional `(in, out)` override when the loaded entity is a clip rather than a sequence. **This amends 015's `effective_source` contract — see "Cross-spec touches" below.**
  - **Display-side wiring (amended 2026-05-21)**: the visible source-side marks — the source monitor's mark bar (`SequenceMonitor:get_mark_in/get_mark_out` consumed by `monitor_mark_bar` at `sequence_monitor.lua:472`) and the source tab (`TimelineTab:get_marks` for kind `"source"`) — read from the same override channel via `effective_source.get_source_marks_for(sequence_id)`. Originally the spec said marks "stay in lockstep automatically" because both consumers read the same columns; only the edit-into-timeline consumer was wired in 019's initial implementation. The display consumers were reading the master source sequence's `mark_in/mark_out` row (always `nil` for a fresh master), so live-bound clips showed no marks. The 2026-05-21 amendment routes both display surfaces through `effective_source.get_source_marks_for` — record-side `SequenceMonitor` and non-source `TimelineTab` instances fall through to the sequence row's persisted marks, preserving staged-mode + record-mode behavior. Pinned by `tests/live/cases/test_live_bound_marks_show_clip_in_out.py`.
  - **`GoToMark*` consumers (amended 2026-05-21)**: `GoToMarkIn`, `GoToMarkOut`, and the positional `GoToMark` alias (`Shift+I`/`Shift+O` in scope `@source_monitor`) must read from the same override channel as the display path. Pre-fix the executors read `seq.mark_in/out` directly from the sequence row, so pressing `Shift+I` while the source viewer showed a live-bound clip parked the playhead at the MASTER source sequence's stale row mark (whatever a prior staged-mode `SetMark` had left there) — violating the "the mark I see is the mark I jump to" invariant. The 2026-05-21 amendment routes `GoToMarkIn/Out` through a shared `get_visible_marks(seq_id)` helper that consults `effective_source.get_source_marks_for` first; the positional `GoToMark` alias delegates to the canonical executors so the override logic lives in exactly one place. Pinned by `tests/live/cases/test_goto_mark_uses_live_clip_in_out.py`.
...
- **FR-024** `keymaps/default.jvekeys`:
  ...
  - **Source-side playhead sync (v2, 2026-05-22).** Every `OpenClipInSourceMonitor` dispatch **match-frame maps** the record-tab playhead into the loaded clip's source frame space, then parks the source tab + source viewer there. The mapping lives in `Clip.owner_frame_to_source(clip, owner_frame)` — `source_in + (owner_frame - sequence_start)` — shared with `MatchFrame` so a single helper carries the math for both commands. The rec playhead and `clip.sequence_start` both live in the rec sequence's frame space; subtracting yields the in-clip offset (rec frames); adding to `clip.source_in` projects into the source media's frame space. Rate-mismatched clips (non-1:1 source↔timeline) are a separate latent concern — see FR-014. Pass-through is via `opts.playhead_frame` on `source_viewer.load_clip`. Asserts: rec engine has a loaded sequence, clip has numeric `sequence_start`/`source_in`. **Naive interpretation rejected**: copying `rec_playhead` verbatim into the master sequence's `playhead_position` is wrong because rec and source sequences do not share a frame space — see 2026-05-22 manual repro where a rec playhead at `00:59:50:01` (frame 89751) was landing the src viewer past the clip's OUT mark.
  - **Focus stays on Timeline.** `OpenClipInSourceMonitor` passes `opts.skip_focus = true` to `source_viewer.load_clip`. The src tab on the timeline panel becomes the displayed tab (existing behavior, driven by `source_loaded_changed`), and is the user-facing readout for the viewer; the user keeps typing into Timeline. Without this, focus jumped to `source_monitor` and the user had to manually click back to keep navigating.
  - **Parking-clamp to clip bounds.** `source_viewer.load_clip` clamps `opts.playhead_frame` into the loaded clip's `[source_in, source_out]` window before writing the master row. Rule (Joe 2026-05-22): rec playhead *before* clip on the timeline → snap to clip IN; *after* → snap to clip OUT. Implemented as a plain min/max on the source-frame value because forward clips (the only kind that reaches this path today — `effective_source._set_source_viewer_clip` asserts `out > in` upstream) have `min=in / max=out`. If reverse-clip support is added later, this clamp needs a direction-aware revisit. Note: **post-load free scrubbing / jogging is NOT clamped** (FR-016a — playhead is a free cursor). The clamp is a parking-only policy.
  - Pinned by `tests/live/cases/test_shift_f_parks_playhead_at_clip_source_in.py` (match-frame mapping + focus-stays-on-timeline + Joe's manual repro: Shift+F, `\``-toggle, move rec, Shift+F → src tracks the new offset).
