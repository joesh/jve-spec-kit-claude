...
- **FR-024** `keymaps/default.jvekeys`:
  ...
  - **Parking-clamp to clip bounds.** `source_viewer.load_clip` clamps `opts.playhead_frame` into the clip's `[min(source_in, source_out), max(source_in, source_out)]` window before writing the master row. The loaded clip IS the user's viewport in live-bound mode, so a load-time park outside that window leaves the viewer rendering a frame the user can't see in their mark window. Common driver: a rec playhead past the clip's timeline extent (most pronounced on double-click of a clip far from the rec playhead). Forward and reverse clips both handled via min/max. Note: **post-load free scrubbing / jogging is NOT clamped** (FR-016a — playhead is a free cursor). The clamp is a parking-only policy.
