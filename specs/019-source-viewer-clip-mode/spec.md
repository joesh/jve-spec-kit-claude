...
- **FR-003** `source_viewer.load_clip(clip_id, opts)` — enters live-bound mode. Loads the clip row + its source sequence (`clip.sequence_id`); binds playback by calling `SequenceMonitor:load_sequence(clip.sequence_id)` (same code path as staged mode); **parks the source-side playhead by writing `master.playhead_position` via `core.playhead.set(clip.sequence_id, target_frame)`** — `transport`'s `playhead_changed` listener then seeks the source engine bound above, so the src tab's ruler (reads `master.playhead_position`) and the source viewer's engine end up at the same frame with one canonical model write. `target_frame = opts.playhead_frame` when supplied; otherwise `clip.source_in`. The Shift+F dispatcher (`OpenClipInSourceMonitor`, FR-024 v2 2026-05-22) supplies `opts.playhead_frame = <record-tab playhead>` so the source tab + viewer track wherever the user is on the record side at the moment of dispatch — same behavior pattern as `F`/MatchFrame. Stashes `clip_id` so the mark-setter and selection_hub publish know to read from clip columns. Transitions the mode flag to `"live_bound_clip"`.
