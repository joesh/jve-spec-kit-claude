...
- **FR-024** `keymaps/default.jvekeys`:
  ...
  - **Source-side playhead sync (v2, 2026-05-22).** Every `OpenClipInSourceMonitor` dispatch reads the record-tab playhead (the `playhead_position` of `transport.record_engine.loaded_sequence_id`) and passes it as `opts.playhead_frame` to `source_viewer.load_clip`. The src tab + source viewer playhead end up at the same frame the record tab was on at the moment of dispatch — mirroring `F`/MatchFrame's "show me the frame I'm on" behavior. Asserts that the record engine has a loaded sequence; in production this is always true at Shift+F time (no rec tab → no clip-on-timeline → no Shift+F target). Pinned by `tests/smoke/cases/test_shift_f_parks_playhead_at_clip_source_in.py` including Joe's manual repro (Shift+F, `\``-toggle, move rec, Shift+F → src tracks moved rec).
