...
- **FR-004a** Source viewer auto-unloads when its currently-loaded entity (clip in live-bound mode; sequence in staged mode) is deleted by any code path. Implementation listens for the existing clip/sequence deletion signals; on receipt, if the deleted id matches the loaded id, calls `unload()` which emits `source_loaded_changed(nil, prev_id)`. Inspector and transport react via that single signal — no separate teardown wiring per listener. No assert on stale access; the model is "viewer is a reactive listener of the model, and the model can change underneath it." NSF posture is preserved by the unload-and-signal contract: the failure case (clip gone, viewer didn't notice) is detectable via a missing signal, not a silent stale read.
- **FR-004b** Source viewer **re-resolves** when its currently-loaded entity is mutated by any non-delete edit (rate change, enabled toggle, rename, source-sequence rename, etc.). Same signal-driven pattern as FR-004a: a single handler listens for clip/sequence mutation signals; on a match against the loaded id, it reloads the clip + its source sequence via `Clip.load` / `Sequence.load`, recomputes the title (FR-016f), re-binds the playback engine if rate/duration changed, and republishes to selection_hub (FR-028). No parallel cache to invalidate. Distinguishes from FR-004a by the signal verb (mutation vs deletion); both share the same listener entry point.
...
- **FR-013** In live-bound mode, setting a new IN or OUT mark dispatches one of two commands via `command_manager.execute_interactive`:
  ...
  The dispatch carries: `clip_id`, `edge` (`"left"` for IN, `"right"` for OUT), `delta_frames` (new_mark − old_mark), `sequence_id` (the clip's owner), `project_id`.
- **FR-014** New command `OverwriteTrimEdge` — peer of `RippleTrimEdge`. Same SPEC.args shape (`clip_id`, `edge`, `delta_frames`, `sequence_id`, `project_id`, all required). Executor mutates exactly one clip row: updates `source_in_frame` (or `source_out_frame`) and `duration_frames` consistent with the delta; recomputes `sequence_start_frame` only if the IN edge moved (out-edge changes leave `sequence_start_frame` unchanged). Owns its own undo entry. No ripple propagation to other clips.
...
- **FR-026** A new Qt timeline binding (in `view_bindings.cpp` or similar — exact location TBD in implementation) routes mouse double-click on a clip rectangle to a Lua handler in `timeline_view_input.lua`. The handler resolves the clip under the mouse and dispatches `OpenClipInSourceMonitor` with that clip's ids.
- **FR-027** Double-clicking on empty timeline space (no clip) is a no-op (returns true, dispatches nothing). Double-clicking on a gap-as-clip is rejected — gaps cannot be loaded into the source viewer; the dispatch returns false with a logged event.
...
- **FR-031** New tests:
  ...
  - `test_source_viewer_load_clip.lua` — live-bound mode entry, mark-setter dispatch to RippleTrimEdge vs OverwriteTrimEdge by trim_mode, key-repeat suppression (FR-016b), Play ignores marks (FR-016e), ClearMarks disabled (FR-016c). Folds the four sub-FR scenarios into one focused source-viewer test.
