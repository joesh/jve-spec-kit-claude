...
- [X] **T023** Wire Qt mouse double-click into the existing Lua handler. *Done 2026-05-20. Implementation landed in `src/timeline_renderer.{cpp,h}`, NOT `src/qt_bindings/view_bindings.cpp` — TimelineRenderer is the timeline-widget class that already owns `mousePressEvent`; adding `mouseDoubleClickEvent` there reuses the same dispatch shape. Refactored `mousePressEvent` body into `dispatchMousePressLikeEvent(event, type_str, callsite)` helper so press + double-click share the event-table-building path (rule 2.5). New override dispatches with `type="double_click"`; Lua `timeline_view_input.handle_mouse` branches on the type. Pure FFI per §2.18 — no business logic in C++. Build clean (commit a70edbdc).*
