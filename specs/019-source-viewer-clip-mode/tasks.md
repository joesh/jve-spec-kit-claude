...
- [ ] **T008 [P]** Live-bound scenario extension in `tests/test_source_viewer_publishes_selection.lua` (EXTEND existing test, do not rewrite). Add: live-bound mode entry publishes `item_type="clip"` with `clip_id`, `project_id`, `sequence_id` (clip's owner). Staged-mode scenarios from the simple-fix stay (do not modify their assertions).

- [ ] **T008d [P]** Failing test in `tests/test_timeline_double_click_dispatches_open_clip.lua` covering FR-026 + FR-027. Simulate the Lua handler receiving a double-click on a clip rectangle; assert `OpenClipInSourceMonitor` was dispatched with the clip's ids. Cover the no-op-on-empty-space case and the gap-as-clip rejection case (FR-027). Tests TDD precondition for T024 (Lua handler portion; T023's C++ binding is exercised by `--test` integration). *(T008a/b/c — covering FR-016b/c/e — folded into T004 (test_source_viewer_load_clip.lua) per the 2026-05-19 scope-trim.)*
