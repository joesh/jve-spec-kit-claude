...
- **FR-011c** (identity discovery — **rewritten by T047; folded into the syncs 2026-06-12**): JVE MUST link an already-imported project's clips to the live Resolve project **automatically inside every sync** (SyncGradesFromResolve / SyncEditsFromResolve run `core/resolve_bridge/discovery.lua` before pulling) — there is NO user-visible connect or stamping command. Clips whose persisted ledger link points at a live timeline item are settled identity and skip matching (their items are pre-claimed). The rest match in priority order: (a) **clip marker** carrying `clip.id` if previously stamped; (b) **content/position** = `(clip name + record-TC + source-TC + media identity)`, the NLE-standard conform key. Unmatched/ambiguous clips MUST be reported (on the sync result's `discovery` table), not silently skipped. Each NEW position match is immediately marker-stamped in the same pass (the FR-012 durability conversion (b) → (a)); a refused stamp (e.g. conflicting customData from another sequence) is reported, never overwritten.
