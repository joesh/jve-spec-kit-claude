...
- Q: Does the live scripting API's timeline-item id equal the DRP-persisted `DbId` (the premise of FR-011b adoption)? → A (refined — supersedes the raw 0/1003 below): **Yes, within a consistent project snapshot.** `TimelineItem:GetUniqueId()` returns the timeline-item's persisted `Sm2Ti DbId` — verified **3/3** on a controlled fresh export (`inbound-findings.md` §2) and **1202/1202** on the live gold project after a V14 regen. The original **0/1003** was an artifact of comparing a **stale, media-managed fixture** (a *different* project instance — only 20/1003 match even positionally) against the live session, NOT a fundamental namespace gap. The empirical 3/3 + 1202/1202 equality **contradicts** the "runtime instance handle ≠ DbId by design" forum claim at the timeline-item level; that claim may still hold at the **media-pool** level, where ids genuinely diverge (line 27). ⇒ FR-011b's id adoption DOES bridge live for a consistent snapshot (the direct-id discovery channel); the marker (FR-011c) is the durable channel because the id churns across re-edits.
...
- **FR-002**: The authored file MUST carry, per clip, a stable identity equal to the JVE `clip.id`, in a field that survives Resolve import and is readable via the scripting API. **Field resolved (2026-05-29 inbound spike, T047): a clip marker carrying `clip.id`** (ASCII, in marker name / `customData`). DRP-persisted `Sm2Ti DbId` carries `clip.id` for **file↔file** re-conform (T004 round-trip) AND bridges to the live scripting API for a consistent snapshot (`TimelineItem:GetUniqueId()` returns that same `Sm2Ti DbId` — 3/3 fresh export, 1202/1202 live; the 0/1003 was a stale cross-instance fixture, see Session 2026-05-29 reconciliation). The marker is the **durable** live channel because the id churns across re-edits; the adopted DbId is the bootstrap channel for the consistent first-connect.
...
- **FR-011b** (inbound identity adoption — **scope corrected by T047**): When importing a Resolve project (DRP/DRT), JVE MUST adopt the Resolve `Sm2Ti DbId` as the JVE `clip.id` when present (else mint a UUID) — mirroring how a master (source clip) adopts the Resolve pool-item `MediaRef DbId` as its `sequences.import_uuid` (**corrected 2026-06-16**: the pool-item id lives on the master, NOT on `media.id` — one physical file can back several source clips, e.g. a synced clip + its plain camera counterpart; see data-model.md "Source-clip identity"). This identity is durable for **file↔file** re-conform (`DbId` round-trips through DRP export→import) AND bridges the live scripting API for a consistent snapshot: `TimelineItem:GetUniqueId()` returns the timeline-item's persisted `Sm2Ti DbId` (3/3 fresh export, 1202/1202 live — the 2026-05-29 "0/1003" was a stale media-managed fixture, a different project instance, reconciled in Session 2026-05-29 above). It is the bootstrap discovery channel (FR-011c (a)). Because the id churns across re-edits, live identity is *also* carried by **a clip marker holding `clip.id`** (FR-011c (b), T048/T049) — the durable channel.
- **FR-011c** (identity discovery — **rewritten by T047; folded into the syncs 2026-06-12**): JVE MUST link an already-imported project's clips to the live Resolve project **automatically inside every sync** (SyncGradesFromResolve / SyncEditsFromResolve run `core/resolve_bridge/discovery.lua` before pulling) — there is NO user-visible connect or stamping command. Clips whose persisted ledger link points at a live timeline item are settled identity and skip matching (their items are pre-claimed). The rest match in priority order: (a) **direct id** = `clip.id == resolve_item_id` — a clip imported from a DRP exported off this same Resolve project carries the Resolve timeline-item id (`Sm2Ti DbId`) as its own `clip.id`. Where that equals the live `TimelineItem:GetUniqueId()`, it is the identity: exact UUID equality, rate-independent, no representation-dependent field. The channel links only the ids that actually coincide; a clip whose id matches no live item falls through. This is the bootstrap channel that re-links a freshly-regenerated project (empty ledger) without any prior stamping — it is what restored grades after the schema-V14 regen wiped `resolve_bridge_link` (observed: 1202/1202 of the project's clips linked by id alone). **Reconciled (2026-06-14):** `GetUniqueId()` returns the timeline-item's persisted `Sm2Ti DbId`, so id equality holds within a *consistent project snapshot* (3/3 fresh export + this 1202/1202 live). The earlier "0/1003" (FR-011b's original wording) was a **stale, media-managed fixture** — a different project instance (only 20/1003 positional) — not a namespace gap; the measured equality contradicts the "runtime handle by design" forum note at the timeline-item level (it may still hold at the media-pool level — see Session 2026-05-29). So this channel is sound: it links the ids of a consistent snapshot and correctly falls through for a stale/cross-instance import (whose ids genuinely shouldn't match). (b) **clip marker** carrying `clip.id` if previously stamped — the durable channel for clips whose live id has churned (re-edit) since export; (c) **content** = source-clip identity (`master.import_uuid` == the live item's pool `import_uuid`) + overlapping source-TC, run BEFORE position so an identity match beats geometry when a clip was moved in Resolve (the grade follows the source clip, not its old slot); same-source duplicates (one identity, several overlapping items) report `duplicate_identity_content` and fall through to (d); (d) **position** = `(track_type, track_index, record_start)` with a name/source_in/media-path content check — the NLE-standard conform key for clips with no source identity (native/compound) and the disambiguator for same-source duplicates; rate-relative, so skipped on a TC-rate mismatch (the content channel is source-TC based and still runs). Unmatched/ambiguous clips MUST be reported (on the sync result's `discovery` table), not silently skipped. Direct-id and marker matches are persisted to the ledger but NOT marker-stamped — the id/marker is already a durable anchor. Each NEW **content or position** match (the no-anchor channels) is immediately marker-stamped in the same pass (the FR-012 durability conversion (c)/(d) → (b)); a refused stamp (e.g. conflicting customData from another sequence) is reported, never overwritten.
...
- **FR-013a**: Deleting a JVE clip MUST remove its stored grade and identity link. **Undoing that delete MUST restore both** — the grade and identity link share the clip's lifecycle (delete cascades both; undo restores both via `Clip.capture_state` / `Clip.restore_state`). A JVE clip whose Resolve item is absent at read-back MUST retain its last-synced grade marked stale, never silently cleared and never shown as current. (Storage mechanism — FK cascade — is in data-model.md.)

**Color model (store + display)**
- **FR-014**: JVE MUST store, per clip, a color grade consisting of a CDL (slope/offset/power + saturation) and/or a LUT reference, plus a fidelity classification (primary / partial / unrepresentable). Grade attaches to the JVE clip. Grades are read-only in JVE (Resolve is the sole authority); JVE provides no grade-editing UI or command, so re-sync overwrites without conflict resolution. LUT references are local filesystem paths (same-machine topology). On the wire `read_grades` also emits `fidelity = "none"` for Resolve items the helper observed but found ungraded (no CDL/LUT/non-CDL tools) — apply drops the JVE `clip_grade` row for that clip (re-sync overwrite of a removed grade). `"none"` is never persisted; absence of a row IS the ungraded state. Empirically discovered 2026-06-04 — the "Resolve emits CDL for every clip" premise that omitted this case was wrong.
- **FR-015**: Reading grades back from Resolve MUST report fidelity honestly: when a Resolve grade exceeds CDL/LUT, the bridge MUST mark it partial/unrepresentable rather than approximating silently. When a Resolve item is observed to have no grade at all the bridge MUST emit `fidelity = "none"` rather than synthesizing an identity CDL. **Honesty extends to display (added 2026-06-19):** a grade can be classified non-primary yet bake to an IDENTITY 3D LUT (a spatial grade — power window/sizing — that a 3D LUT cannot carry), so the viewer shows passthrough. Storing that as "graded" is a silent failure (rule 2.32). The bridge therefore computes a per-clip **`reproduction`** axis at sync time — what JVE can actually DISPLAY: `full` (primary CDL) / `approximate` (non-primary shown via a non-identity LUT) / `not_shown` (identity-LUT or no carrier). Only **`not_shown`** earns a visible flag (refined 2026-06-19): `approximate` sits on nearly every clip in a real project (1016/1043 on Anamnesis) and the look is mostly right, so flagging it is noise, not signal — the value is still recorded and remains findable, but it shows no caption/stripe/tooltip/inspector notice. `not_shown` is surfaced as a right-aligned plain-language status in the monitor's TITLE STRIP (revised 2026-06-19 — a caption bar BELOW the image letterboxed the frame and reserved a row even when empty; the title strip is existing chrome and costs no image real estate; NOT on the Metal surface — child overlays occlude Metal on macOS), a thin red colour stripe on the clip's bottom edge in the timeline with a hover tooltip carrying the sentence, the Inspector text, and a findable/filterable clip attribute (Find). Wording is plain (no codes): "Ungraded here — this shot is graded in Resolve only". Identity detection is an early-out `.cube` reader (`core/lut_identity.lua`); classification is `ClipGrade.classify_reproduction`. Cube parsing accepts scientific-notation values (Resolve writes near-zero coords like `6.10361e-05`); the parser matches whitespace-separated tokens and validates via `tonumber`, never a number-shaped char class that drops the exponent sign. See data-model `clip_grade.reproduction`.

**Upper-track composite layers (fixed 2026-06-19):** the upper/overlay video tracks (V4+) — composite + power-window layers, which Resolve holds as Fusion-wrapped shots with no indexable source range — were systematically NOT graded and so couldn't be flagged: `read_grades` silently `continue`d on any item whose live `GetSourceStartFrame()` is non-integer (its non-media gate), while `read_identities` (which builds the ledger) has no such gate, so those items got LINKED but never received a grade row (159 such clips on Anamnesis, ~all V4/V5/V6, 0 of V1–V3 — a rule-2.32 silent drop). `read_grades` now, for a non-media item that still has a media-pool item (a Fusion-wrapped shot — distinct from a pure generator/Text+/adjustment, which has none and stays skipped), emits `{resolve_item_id, fidelity:"unrepresentable"}` (no cdl, no lut) instead of dropping it; ExportLUT is never attempted (no source to bake). `apply` joins it through the ledger and writes a `not_shown` grade, so the linked JVE clip is honestly flagged. The non-int gate is NOT offline-media (offline keeps its source range). See memory `todo_023_read_grades_skips_upper_track_items`.
- **FR-016**: JVE's viewer MUST display the stored primary grade (apply CDL, then LUT if present); display MUST be pull-based from model state (MVC), not dependent on an imperative push. A completed sync emits `grades_changed`; a parked monitor re-pulls (clears the seek dedup and re-renders the parked frame) so the new look appears WITHOUT the user moving the playhead. **Sync progress (added 2026-06-19):** the bake+pull runs minutes on a large timeline, so the command emits `grade_sync_started` at entry and the existing `sync_grades_from_resolve_completed` at exit (success AND error); the monitor shows "⟳ Syncing grades from Resolve…" in the title strip between them, so the editor knows the on-screen look is provisional. (NOTE: if `grades_changed` is emitted only at the END of `apply`, any assert in the per-clip loop — e.g. the cube-parse bug fixed this date — aborts the async tail before the signal fires, leaving the parked frame stale until a manual seek. Keep that emit reachable.) The "fires success AND error" guarantee for `sync_grades_from_resolve_completed` is what clears the "⟳ Syncing…" indicator, so it MUST hold even when the async apply phase hits an internal-invariant assert (e.g. a baked cube missing from disk in `new_grade_from_response_row`). The C++ socket boundary that delivers the helper response SWALLOWS Lua errors raised on its callback (`jve_invoke_lua_callback` → `jve_handle_lua_callback_error` logs+pops+continues, never re-raises), so an un-caught async assert would never reach `notify()` and the indicator would hang until restart. Therefore the read_grades response handler pcall-wraps the apply phase and routes a caught fault through the one terminal `notify(..., "apply_failed", err)` path — loud failure, no fallback values, distinct from `resolve_api_error` (origin fidelity).
- **FR-017**: Applying read-back grades MUST go through the command system (undoable), restoring prior grade state on undo. Attribution of helper rows to JVE clips MUST be JVE-side: the helper emits rows keyed on its native `resolve_item_id`, and the apply layer joins via `identity_ledger.lookup_clip_id` (populated by the sync's own auto-discovery per FR-011c, which runs before the grade pull in the same command). This honors FR-021 — helper holds no JVE state. Marker stamping (the FR-012 durability channel for surviving Resolve-side cuts/splits) also happens inside discovery, for newly position-matched pairs only — it is never a prerequisite for any sync.
...
- **FR-023**: All bridge actions — send-to-Resolve, connect-imported-project, sync-grades, sync-edits — MUST be user-invocable through the command system (menu / shortcut / programmatic), consistent with how every JVE operation is dispatched. No bridge action is a hidden or implicit side effect.
  ...
  - **Regression gate.** `tests/live/cases/test_bridge_menu_dispatch.py` clicks each Color menu item through real OS input and asserts BOTH (a) no forbidden marker (`LUA CALLBACK ERROR`, handler failure, raw assert) lands in the suite-log slice, AND (b) `bridge_completion_count(op_name)` advanced by at least one. The counter-advance assertion catches the regression class "no log marker, but pcall silently swallowed the async tail" — which "no Lua error" alone CANNOT.
  - **Open items.** (Resolved 2026-06-12: `ConnectToResolveProject` no longer exists — its discovery folded into the syncs (`core/resolve_bridge/discovery.lua`) and its stamping became automatic for new position matches, so the sync-return revisit is moot. The bridge menu carries three commands; the menu smoke covers all three.)
