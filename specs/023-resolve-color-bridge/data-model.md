...
- **Synced-audio alignment is offset-based, PER CHANNEL — not timecode-based.** Resolve stores a `SampleOffset` (file sample that plays under the video's first frame) per channel in the synced clip's audio `FieldsBlob`; a free-run field recorder is never jam-synced to the camera, so their timecodes do not coincide. **A camera clip's blob records the whole "Auto Sync Audio" ANALYSIS — every overlapping sound-roll Resolve evaluated, each with its own `SampleOffset` — but only ONE is the SELECTED sync (the Media Pool's "Synced Audio" column).** Resolve serializes the selected source LAST among the external (non-camera-scratch) refs — its channels are the per-group terminators, so its final channel is the last external `MediaRef` before the camera's own embedded scratch (always trailing). `resolve_synced_audio_linkage` (`drp_importer`) therefore attaches **only the owning pool item of the last external ref** (`selected_synced_audio_pool_id`); the earlier externals are rejected candidates. Verified against the anamnesis corpus: 4/4 multi-candidate clips (C033→290-T001, C031→342-T001, C034→290-T002, C017→S028-T003) match Resolve, and all 382 single-candidate clips are trivially consistent. Attaching every candidate (the prior bug) grew 5–8 spurious audio tracks per clip and cascaded false "media too short" relink failures. `drp_binary.extract_media_ref_sample_offsets` decodes them (index-aligned with `extract_media_refs`); the importer threads them as `synced_audio_streams = {{media_id, sample_offsets={…per channel}}}` (`importer_core`), and `master_builder.add_synced_audio_streams` places each sync channel **co-located with the video** (`sequence_start_frame = video_tc`, record length = video take) with `source_in_frame = audio_tc + SampleOffset`. Resolve **omits** the field when the offset is zero (jam-synced / no slip) ⇒ absent means 0 (boundary normalization, not a fallback). When the master has **no video stream** (offline/undecoded video — nothing to co-locate against), synced audio anchors at its own TC spanning the whole file and the offset is not applied until the video is present. **OPEN (needs Joe's eyeball in live Resolve):** whether an absent `SampleOffset` truly means 0-offset co-location vs. some other default — see [[todo_023_synced_relink_extent_fix]].
- **One audio clip per source channel — the channel is a media_ref selector, decoded end-to-end.** A multichannel audio file is N streams; each maps to its own master track (synced/Adaptive expand). `media_refs.source_channel` (0-based; NULL for video; required ≥0 for AUDIO refs via `MediaRef.create`) names which file channel a ref reads. **How many channels a media has is the file's TRUE embedded-audio channel count — the SUM of `TracksBA.NumChannels` across the pool item's own `BtAudioInfo` (`drp_importer.count_embedded_audio_channels`), NOT the number of `BtAudioInfo` elements.** A camera file (BRAW/MOV) packs all channels into ONE interleaved `BtAudioInfo` whose `NumChannels` is the real count (a 2-channel BRAW is one BtAudioInfo carrying 2) — counting elements undercounted stereo camera clips to a single mono track and silently dropped channel 1. `add_audio_streams` then fans `audio_channels` into one "Embedded N" master track per channel. The resolver emits one entry per track carrying that channel; `tmb_clip_builder` forwards `source_channel` into the TMB clip; the C++ decode path (`ClipInfo.source_channel` → `Reader::DecodeAudioRangeUS` → `FFmpegResampleContext::init`) honors it via an swr remap matrix that routes that one source channel to BOTH stereo outputs (dual-mono monitoring). The **BRAW** path bypasses FFmpeg/swr (it reads native interleaved F32 from the SDK), so `decode_braw_audio_range` honors `source_channel` directly — a deinterleave of that one channel into dual-mono stereo (an out-of-range channel surfaces as an Error, never a silent flat envelope). `source_channel = -1` is the composite/**Adaptive** default: swr's downmix of all channels (a stereo source stays L→L/R→R; the BRAW composite keeps the native channels, folded downstream). Output is always stereo regardless; the channel only selects the source. A per-clip Reader pool key (`(track, clip_id)`) guarantees one channel per resampler, so the decode cache needs no channel key. Before this, the decoder dropped the channel and downmixed every track identically — soloing any synced track played the same composite. Per-channel **waveforms** match: the peak pipeline is keyed by `(media_id, source_channel)` — `PeakGenerator::RequestPeaks` takes a `source_channel` and decodes through the same swr remap, `peak_cache` uses a job key (`<media_id>` for composite, `<media_id>__ch<N>` for one channel) that doubles as the `.peaks` filename stem, `init_for_project` pre-generates exactly the channels referenced by AUDIO media_refs (`Media.get_audio_channels_for_project`), and the timeline renderer passes the clip's `resolved_media.source_channel` to `get_visible_peaks`. So each track's waveform reflects its own channel, not the composite. See [[todo-023-per-channel-audio-streams]].
...
**Schema:** adds `sequences.import_uuid TEXT` (nullable; masters only). `media.file_uuid` is **dropped** (2026-06-17) — `media.id` IS the DRP `MediaRef DbId` (stable per file), so a separate identity column duplicated the primary key; `file_path UNIQUE` enforces one media per physical file.

**Outbound (landed 2026-06-16):** the exporter keys the DRT media-pool by the source-clip identity, not the file. `payload_builder` resolves each timeline clip to its master and emits one pool item per master, carrying `file_uuid = master.import_uuid or master.id` (native masters fall back to `master.id`) with the underlying file's path/rate/duration/TC. Two masters over one `.mov` ⇒ two pool items, same path, distinct DbId — the dual of the inbound model, so the synced/plain distinction survives the round-trip. For DRP-imported masters `import_uuid` == the original `MediaRef DbId`, so single-pool-item round-trip bytes are unchanged. The timeline clip's `<MediaRef>` carries the same master identity. `drt_writer` is unchanged (it reads the payload's `file_uuid`/`media_uuid`, whose *values* moved from `media.id` to the master identity). `media.file_uuid` (the column) has been **dropped** (2026-06-17) — it had no behavioral reader, media dedup is path-based, and identity is `media.id`. **Inbound content-match (wired 2026-06-17, Option I):** `discovery.match` now runs a `content` channel keyed on `master.import_uuid` + source-TC overlap, in priority order **direct-id → marker → content → position**. Content runs BEFORE position so a clip the colorist moved in Resolve follows its source clip, not its old slot (identity beats geometry); same-source duplicates report `duplicate_identity_content` and fall through to position to disambiguate by record_start. The helper's `read_timeline` emits each item's `import_uuid` for this. The dead `identity_ledger.reconcile` (and its `blade_inherit` pass, with no live analog) was deleted — the live matcher subsumes it. Verification gate CLOSED (2026-06-17): the probe (`probe_mp_item_identity.py` vs `anamnesis-gold-timeline.drp`) confirmed `MediaPoolItem.GetUniqueId()` == `Sm2MpVideoClip@DbId` == `import_uuid` (∩ DbId = 15, ∩ `UniqueMediaPoolItemId` = 0; `GetMediaId()` is the other family) — see phase0-findings §K1a. The content channel is live-correct.
