...
- **FR-002**: System MUST provide a Preferences toggle that disables all backend communication (registration, heartbeat, submission) and MUST honor it immediately without restart. When the toggle is flipped from disabled to enabled and no install record exists yet, the next backend interaction MUST perform `/register` first. The toggle MUST live in a discoverable Privacy panel (bound to Cmd+, by default) that also exposes the install identity (install_id, jve_sha_at_register, consent_version, consent_accepted_ts) and a Revoke button that deletes `~/.jve/install_id.json` so the next launch re-prompts for consent.
- **FR-002a**: The consent text is versioned (`CONSENT_VERSION` integer in `bug_reporter.consent`). When System detects that an existing install record's `consent_version` is older than the current `CONSENT_VERSION`, the next telemetry init MUST re-prompt with the new consent text BEFORE resuming any backend communication. Accepting writes the new version + timestamp back to the install record; declining is treated as a fresh decline (toggle off, no backend traffic).
