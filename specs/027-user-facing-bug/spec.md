...
24. **Given** the Worker returns a malformed or non-JSON response, **When** the app receives it, **Then** the app asserts with an actionable error citing the endpoint and the unparseable response. The app does NOT silently treat the response as success or failure.
...
- **FR-021a**: If the backend returns a malformed or non-JSON response, System MUST assert with an actionable error citing the endpoint and the unparseable response. System MUST NOT silently treat malformed responses as success or generic failure.
