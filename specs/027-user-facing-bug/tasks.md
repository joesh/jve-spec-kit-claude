...
**Date**: 2026-06-25 (revised after adversarial review same date) · 2026-06-27 sync.

## Status as of 2026-06-27

All implementation tasks T001–T054 **shipped** to branch `027-user-facing-bug`
across June 2026 (commits range from the early Phase-A work through the
post-pass adversarial-review batch ending 2026-06-27). The audit-review
follow-up items the original tasks list did not capture (F12 async slideshow,
QTimer parent + deleteLater, qt_compat fail-fast sweep, Privacy panel +
consent v2 + re-prompt, FR-021a async classified-result, pixel-side
redaction) **also shipped** as a separate commit cluster on 2026-06-27 —
see `git log --oneline 027-user-facing-bug` for the per-commit history;
the spec.md FRs are kept current there.

T055 + T056 (validation walkthrough + regression) **remain pending Joe's
smoke test** — they are user-driven, not implementation work.

Per-task commit hashes are not chased into this file because (a) several
tasks landed across multiple commits as the adversarial review surfaced
gaps, (b) the source of truth for the "what changed when" question is
`git log`, not this checklist. The boxes below are flipped to `[x]` to
acknowledge the production reality; granular history lives in git.
...
- [x] **T001** Wire build-time git SHA injection (build-time, NOT configure-time).
...
- [x] **T001a** Create `tests/fixtures/signature_vectors.json` (shared by T002 Lua and T020 TS — was T019 in prior pass, moved here per ordering).
...
- [x] **T002 [P]** Write failing test `tests/synthetic/lua/test_bug_reporter_signature.lua`.
...
- [x] **T003 [P]** Write failing test `tests/synthetic/lua/test_bug_reporter_capture_monotonic.lua`.
...
- [x] **T004 [P]** Write failing test `tests/synthetic/lua/test_bug_reporter_capture_main_window.lua`.
...
- [x] **T005 [P]** Write failing test `tests/synthetic/lua/test_bug_reporter_artifact_shape.lua`.
...
- [x] **T006 [P]** Write failing test `tests/synthetic/lua/test_bug_reporter_dialog_wiring.lua`.
  - Requires `--test` mode (absolute path).
...
- [x] **T007** Add SHA-256 binding in NEW file `src/bug_reporter/crypto_bindings.cpp`.
...
- [x] **T008** Implement `src/lua/bug_reporter/signature.lua` (NEW file).
...
- [x] **T009** Fix `src/lua/bug_reporter/capture_manager.lua`.
...
- [x] **T010a** Set `objectName("JVEMainWindow")` on the JVE main window.
...
- [x] **T010b** Fix `src/bug_reporter/qt_bindings_bug_reporter.cpp` `lua_grab_window` + remove log spam.
...
- [x] **T011** Fix `src/lua/bug_reporter/json_exporter.lua` + add filesystem bindings.
...
- [x] **T012 [P]** Create `src/lua/bug_reporter/ui/submission_state.lua` (NEW file).
...
- [x] **T013** Rewrite `src/lua/bug_reporter/ui/submission_dialog.lua` as a view.
...
- [x] **T014a** Add a generic zip primitive — recon R confirms no general-purpose zip binding exists today (`qt_zstd_compress` is DRT-specific).
...
- [x] **T014c** Modify `src/lua/core/commands/report_bug.lua` for Phase A submit handler.
...
- [x] **T015** Delete Phase-A-safe legacy files (concrete list per recon R-08).
...
- [x] **T016** Walk Phase A quickstart end-to-end.
...
- [x] **T017** Scaffold `bug-reporter-worker/` TypeScript project.
...
- [x] **T018** Author migration `bug-reporter-worker/migrations/0001_initial_schema.sql`.
...
- [x] **T020 [P]** Write failing `bug-reporter-worker/test/register.test.ts`.
...
- [x] **T021 [P]** Write failing `bug-reporter-worker/test/heartbeat.test.ts`.
  - Implements every bullet in [contracts/heartbeat.md § Contract test outline](./contracts/heartbeat.md).

- [x] **T022 [P]** Write failing `bug-reporter-worker/test/report.test.ts`.
...
- [x] **T023 [P]** Write failing `bug-reporter-worker/test/promote.test.ts`.
...
- [x] **T024 [P]** Write failing `tests/synthetic/lua/test_bug_reporter_install_persist.lua`.
...
- [x] **T025 [P]** Write failing `tests/synthetic/lua/test_bug_reporter_transport_classify.lua`.
...
- [x] **T026 [P]** Write failing `tests/synthetic/lua/test_bug_reporter_queue_cap_and_drain.lua` (renamed from prior `_queue_cap` — now covers both).
...
- [x] **T027 [P]** Write failing `tests/synthetic/lua/test_bug_reporter_payload_clamp.lua`.
...
- [x] **T028 [P]** Write failing `tests/synthetic/lua/test_bug_reporter_consent_gate.lua`.
...
- [x] **T029 [P]** Write failing `tests/synthetic/lua/test_bug_reporter_telemetry_disabled.lua`.
...
- [x] **T029a [P]** Write failing `tests/synthetic/lua/test_bug_reporter_hardware_resnapshot.lua` (NEW per adversarial review).
...
- [x] **T030 [P]** Extend `src/bug_reporter/crypto_bindings.cpp` (created in T007).
...
- [x] **T031 [P]** Create `src/bug_reporter/hardware_bindings.cpp` + `hardware_bindings.mm` (mac).
...
- [x] **T032 [P]** Create `src/bug_reporter/http_bindings.cpp` (NEW file). **[P] — separate file, no shared-file conflict with T030/T031.**
...
- [x] **T033 [P]** Create `src/lua/bug_reporter/install.lua`.
...
- [x] **T034 [P]** Create `src/lua/bug_reporter/hardware_snapshot.lua`.
...
- [x] **T035 [P]** Create `src/lua/bug_reporter/transport.lua`.
...
- [x] **T036 [P]** Create `src/lua/bug_reporter/pending_queue.lua`. **[P] — separate file from T035; tests T026 mock transport directly so no inter-module dependency at file level.**
...
- [x] **T037** Create `src/lua/bug_reporter/telemetry.lua` (depends on T033 + T034 + T035 + T036).
...
- [x] **T038 [P]** Create `src/lua/bug_reporter/ui/consent_dialog.lua`.
...
- [x] **T039 [P]** Create `src/lua/bug_reporter/ui/pref_privacy.lua` + integrate.
...
- [x] **T040 [P]** Create `bug-reporter-worker/src/signature.ts`.
...
- [x] **T041 [P]** Create `bug-reporter-worker/src/d1.ts`.
...
- [x] **T042 [P]** Create `bug-reporter-worker/src/auth.ts`. **[P] — separate file from T040/T041/T043/T044.**
...
- [x] **T043 [P]** Create `bug-reporter-worker/src/r2.ts`.
...
- [x] **T044 [P]** Create `bug-reporter-worker/src/github.ts`.
...
- [x] **T045** Create `bug-reporter-worker/src/index.ts` with router skeleton + `/register` handler.
...
- [x] **T046** Add `/heartbeat` handler to `bug-reporter-worker/src/index.ts`.
...
- [x] **T047** Add `/report` handler to `bug-reporter-worker/src/index.ts`.
  - Algorithm:
...
- [x] **T048** Add `/promote` handler to `bug-reporter-worker/src/index.ts`.
...
- [x] **T049 [P]** Extend `src/lua/bug_reporter/ui/submission_dialog.lua` + `submission_state.lua` for Phase B.
  - State adds:
...
- [x] **T050 [P]** Add `telemetry.init()` to app startup. **[P] with T049: separate files.**
...
- [x] **T051** Update `src/lua/core/commands/report_bug.lua` for Phase B.
...
- [x] **T052** Delete remaining legacy files.
...
- [x] **T053 [P]** Create `bug-reporter-worker/triage-promote.html`.
...
- [x] **T054 [P]** Create `docs/bug-reporter-ops.md`.
...
- [x] **T-NEW-B** Author `specs/027-user-facing-bug/consent-text-v1.md`.
...
- [x] **T055** Walk Phase B quickstart end-to-end per [quickstart.md § Phase B](./quickstart.md). Capture TSO; verify pass criteria.

- [x] **T056** Re-run Phase A quickstart for regression.
