...
- **FR-008a**: `transport_target` MUST persist per-project (stored alongside other per-project UI state such as `active_sequence_id` and `displayed_tab_id`). On project open the persisted value MUST be restored. If no persisted value exists for the project (first open after this refactor lands, or a freshly created project), `transport_target` MUST default to `record`. There is no other implicit default; the persistence read and the explicit `record` fallback are the only two ways `transport_target` is initialized.
