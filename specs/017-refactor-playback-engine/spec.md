...
- **FR-008a**: The transport target is **derived** from persisted UI state — it is NOT itself stored under a `transport_target` key (superseded 2026-05-19; see [plan.md §10](./plan.md) and [data-model.md §15](./data-model.md)). The persisted inputs are: (a) `last_focused_panel` (per-project setting written by focus_manager on every focus change) and (b) `displayed_tab_kind` (per-project setting written by timeline_state on every `switch_to_source_tab` / `switch_to_record_tab`). On project open, both are restored before any transport query runs; the next `transport.get_target()` call computes `source` or `record` from them. First-open default — no `last_focused_panel`, no `displayed_tab_kind` — falls out as `record`, matching `transport.get_target()`'s closing branch.
