...
**Effective source — recency rule.** The "source" feeding patch routing is the **effective source**, picked from two independent inputs:
...
The rule is **whichever of these two panels was activated most recently wins**, provided it has a value. If the recency winner has no value, the other (if any) is the answer. Activating any *other* panel (timeline, inspector, …) does **not** change the recency winner — so clicking a src-btn on the timeline shifts focus there but the previously-picked source survives the focus shift (otherwise drag-from-src-btn would be impossible). Empty browser selection or unloaded source viewer simply means "no value from that input"; the other still wins.
