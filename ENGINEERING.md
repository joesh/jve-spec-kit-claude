...
- **2.29**: Set sequence_id on Timeline Commands - All commands that modify a sequence MUST include `sequence_id`. The command manager derives snapshot targets from this automatically. Without `sequence_id`, snapshots won't be created and undo may behave incorrectly.
...
- **2.34**: Test Domain Behavior, Not Implementation - Describe expected test behavior without naming any function, variable, or module. If you can't, you're testing implementation. Derive expected values from domain requirements (timecode math, NLE conventions, what the user sees/hears), NEVER by tracing the code. If you computed the expected value by reading the implementation, the test just verifies the code does what the code does. Ask Joe "what should happen here?" rather than reading the code and deciding yourself.
