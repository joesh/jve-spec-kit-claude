...
void registerQtBindings(lua_State* L)
{
    ...
    lua_pushcfunction(L, lua_set_layout); lua_setfield(L, -2, "SET_WIDGET_LAYOUT");
    ...
    lua_pushcfunction(L, lua_install_drop_target); lua_setfield(L, -2, "INSTALL_DROP_TARGET");
    lua_pushcfunction(L, lua_synthetic_drop); lua_setfield(L, -2, "SYNTHETIC_DROP");
    ...
    lua_pushcfunction(L, lua_install_drop_target); lua_setglobal(L, "qt_install_drop_target");
    lua_pushcfunction(L, lua_synthetic_drop); lua_setglobal(L, "qt_synthetic_drop");
    ...
    // Live keyboard-modifier query (spec 025 FR-005 Alt+click on M/S buttons)
    lua_pushcfunction(L, lua_keyboard_modifiers);     lua_setglobal(L, "qt_keyboard_modifiers");
    ...
    lua_pushcfunction(L, lua_set_splitter_drag_handler); lua_setglobal(L, "qt_set_splitter_drag_handler");
    lua_pushcfunction(L, lua_set_scroll_area_v_user_scroll_handler); lua_setglobal(L, "qt_set_scroll_area_v_user_scroll_handler");
    ...
    lua_pushcfunction(L, lua_link_scroll_areas_h); lua_setglobal(L, "qt_link_scroll_areas_h");
    ...
    lua_pushcfunction(L, lua_set_action_shortcut); lua_setfield(L, -2, "SET_ACTION_SHORTCUT");
    ...
    lua_pushcfunction(L, lua_set_app_state_handler); lua_setfield(L, -2, "SET_APP_STATE_HANDLER");
