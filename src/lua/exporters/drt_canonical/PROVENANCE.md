...
- `full_reference_*.xml` — single-element extracts from
  `tests/fixtures/resolve/resolve_authored_full.drp` (the kitchen-sink
  dissection fixture, hand-authored in Resolve UI). Borrowed verbatim
  because their `FieldsBlob` payloads are zstd-compressed,
  Resolve-version-stamped, and not yet decoded
  (phase0-findings.md §K).
...
| `full_reference_mp_video_clip_a005.xml` | outer `Sm2MpVideoClip@DbId` (→ media.file_uuid), embedded `<MpFolder>` back-ref (→ minted mp_folder DbId), `<UniqueMediaPoolItemId>`, `<Name>` if payload media's basename differs from `A005_C052_0925BL_001.mp4` | Everything else — FieldsBlob (zstd), VirtualAudioTracksBA, BtVideoInfo (FieldsBlob/Clip/Time/Geometry/Radiometry/Proxy/VideoMetadata blobs), embedded BtAudioInfo, file_path baked inside Clip/TracksBA blobs, internal DbIds for SmPTZRPreset/BtVideoInfo/BtAudioInfo. Source: `resolve_authored_full.drp` MpFolder.xml lines 238–290 (the A005 entry). |
