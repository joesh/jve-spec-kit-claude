...
local function try_import_media_item(media_item, project_id, project_settings,
    ...
    -- parser extracts one (currently the DRP MediaRef DbId; other formats
    -- mint a UUID). Stable ids across re-imports keep per-media-id caches
    -- (peak files, future content caches) intact instead of orphaning them
    -- every time the DRP is re-converted into the same destination.
