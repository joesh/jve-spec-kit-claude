...
local function tile_style(state)
    local bg, border = "#3a3a3a", "#555"
    local border_width = 1
    if state.placeholder then
        bg, border = "#2a2a2a", "#3a3a3a"
    elseif state.modifier_active then
        bg, border = "#2563cc", "#3a7adf"  -- blue when modifier currently pressed
    elseif state.modifier then
        bg, border = "#3a3a3a", "#666"     -- modifier tile, idle
    elseif state.binding then
        if state.binding.has_global and state.binding.has_panel then
            bg, border = "#5a3a82", "#7a52a8"
        elseif state.binding.has_global then
            bg, border = "#5a3a82", "#7a52a8"  -- purple
        else
            bg, border = "#2f6b3a", "#43935a"  -- green
        end
    end
    if state.selected then
        border = "#4a9eff"  -- cyan-blue selection outline
        border_width = 2
    end
...
local function build_tile(cell)
    ...
    PROP.SET_STYLE(cmd_label, "QLabel { font-size: 10px; color: #ffffff; font-weight: 600; }")
    ...
    PROP.SET_STYLE(key_label, "QLabel { font-size: 11px; color: #a8b4c4; font-weight: 500; }")
