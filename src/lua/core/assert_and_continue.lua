...
--- 2026-06-27). `assert_and_continue.check` logs the violation with
--- a trace and lets the caller drop the offending item.
---
--- Default policy is fail-fast (ENGINEERING.md 1.14). Use
--- `assert_and_continue.check` ONLY when the call site has a
--- documented "what we do when this fails" branch immediately
--- following the check. If there isn't one, use `assert`.
