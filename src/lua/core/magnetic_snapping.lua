...
-- Returns array of {time, type, description} where time is integer frame
function M.find_snap_points(state, excluded_clip_ids, excluded_edge_specs, opts)
    ...
    -- Add playhead position as snap point (integer frame)
    local playhead_time = state.get_playhead_position()
    assert(type(playhead_time) == "number", "magnetic_snapping: playhead must be integer frame")
...
function M.find_closest_snap(state, target_time, excluded_clip_ids, excluded_edge_specs, viewport_width_px, opts)
    ...
    assert(type(target_time) == "number", "magnetic_snapping.find_closest_snap: target_time must be integer frame")
    ...
    for _, snap_point in ipairs(snap_points) do
        assert(type(snap_point.time) == "number", "magnetic_snapping: snap_point.time must be integer frame")
...
-- Returns adjusted time (integer frame) and snap info {snapped: boolean, snap_point: {...} or nil}
