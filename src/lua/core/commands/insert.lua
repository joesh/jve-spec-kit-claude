--- Insert command - wrapper for AddClipsToSequence(edit_type="insert")
--
-- Responsibilities:
-- - Load masterclip sequence and resolve stream timing
-- - Delegate to AddClipsToSequence for actual insertion
--
-- Invariants:
...
function M.register(command_executors, command_undoers, db, set_last_error)
    ...
    command_executors["Insert"] = function(command)
        ...
        log.event("Executing Insert command (via AddClipsToSequence)")
        ...
        if not master_clip_id or master_clip_id == "" then
            ...
            assert(master_clip_id, "Insert command: no master clip selected in browser")
        ...
        assert(source_sequence, string.format(
            "Insert command: masterclip %s not found", master_clip_id))
        assert(source_sequence:is_masterclip(), string.format(
            "Insert command: sequence %s is not a masterclip (kind=%s)",
        ...
        assert(media_id and media_id ~= "", string.format(
            "Insert command: masterclip %s has no media_id in streams", master_clip_id))
        ...
        assert(video_timing or audio_timing, string.format(
            "Insert command: masterclip %s has no video or audio streams", master_clip_id))
        ...
        assert(#clips > 0, string.format(
            "Insert command: no clips to insert for masterclip %s", master_clip_id))
        ...
        if nested_cmd and nested_cmd.get_parameter then
            ...
            -- the outer Insert and duplicate every inserted clip. Same shape
            -- as Overwrite's forwarding — see overwrite.lua.
