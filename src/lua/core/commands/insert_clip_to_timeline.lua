...
local SPEC = {
    ...
    args = {
        clip_id = {},  -- Generated by executor if not provided
        duration_frames = {},
        master_clip_id = {},
        media_id = {},
...
function M.register(command_executors, command_undoers, db, set_last_error)
    command_executors["InsertClipToTimeline"] = function(command)
        ...
        local raw_timeline_start_frame = args.timeline_start_frame
        ...
        if not sequence then
            print(string.format("ERROR: InsertClipToTimeline: Sequence %s not found.", tostring(args.sequence_id)))
            return false
        end
        ...
        if not media_id or media_id == "" then
            set_last_error("InsertClipToTimeline: Missing media_id (and no usable master_clip_id)")
            return false
        end
        ...
        if args.duration_frames then
            clip_duration_rational = Rational.new(args.duration_frames, sequence_fps_num, sequence_fps_den)
        end
        if args.source_in_frame then
            clip_source_in_rational = Rational.new(args.source_in_frame, sequence_fps_num, sequence_fps_den)
        end
        if args.source_out_frame then
            clip_source_out_rational = Rational.new(args.source_out_frame, sequence_fps_num, sequence_fps_den)
        end

        -- Ensure duration is positive (STRICT)
        if clip_duration_rational.frames <= 0 then
            set_last_error("InsertClipToTimeline: Calculated duration is zero or negative.")
            return false
        end
        
        -- Ensure source_out is greater than source_in
        if clip_source_out_rational.frames <= clip_source_in_rational.frames then
            set_last_error("InsertClipToTimeline: Calculated source_out is not greater than source_in. Adjusting.")
        ...
        local function insert_clip(_, payload, target_track, pos)
            ...
            local clip = Clip.create(payload.clip_name or "Clip", payload.media_id, {
                ...
                fps_numerator = sequence_fps_num,
                fps_denominator = sequence_fps_den,
                enabled = true,
            ...
            assert(clip:save(), "InsertClipToTimeline: failed to save clip to timeline")
        ...
        print(string.format("✅ Inserted clip %s to track %s at %s", args.clip_id, track_id, tostring(timeline_start_rational)))
        return true
    end
    ...
    command_undoers["UndoInsertClipToTimeline"] = function(command)
        ...
        if not clip then
            print(string.format("WARNING: UndoInsertClipToTimeline: Clip not found: %s", args.clip_id))
            return false
        end

        command_helper.delete_properties_for_clip(args.clip_id)
        if clip:delete() then
            print(string.format("✅ Removed clip %s from timeline", args.clip_id))
            return true
        else
            set_last_error("Failed to delete clip from timeline")
            return false
        end
    end
