...
- LuaJIT 2.1 (JVE model/command/UI/exporter + all bridge policy: supervision, protocol, correlation); C++17 / Qt6 only for thin one-to-one FFI (`qt_process_*`, `qt_local_socket_*`, `qt_zstd_compress`) and the renderer CDL stage — no Resolve-specific or supervision logic in C++ (2.18/1.10). Helper process: **Python** (resolved by Phase-0 spike — external LuaJIT segfaults loading `fusionscript.so`; see `specs/023-resolve-color-bridge/phase0-findings.md`) — invisible to JVE behind the socket. + Qt6 Network (`QLocalSocket` client + `QProcess`, already linked per spec 020; `debug_terminal` is the server-side pattern to mirror), SQLite via lsqlite3, libzstd (existing `qt_zstd_decompress`; add `qt_zstd_compress`), dkjson (wire JSON), DaVinci Resolve Studio scripting API (`fusionscript` — **helper process only**, never linked into JVE). (023-resolve-color-bridge)
...
- **2.8**: ALWAYS commit with proper attribution. The assisting LLM MUST name its own model, version, and reasoning effort — never a bare "Claude"/"Codex":
  `Authored-By: Joe Shapiro <joe@shapiro.net> With-Help-From: <model> <version> (<variant if any>, effort=<level>)`
  Example: `Authored-By: Joe Shapiro <joe@shapiro.net> With-Help-From: Claude Opus 4.8 (1M context, effort=high)`. Fill in YOUR actual model/version/effort at commit time; if effort isn't known, omit the `effort=` clause rather than guessing.
